"""Evaluation-only Stage P runner using the approved incremental/cache controller."""
...
def _types():
    ...
    controller=_load(prefix+"stage_p_callback_controller_v1",semantic_root/"stage_p_callback_controller_v1.py")
    lifecycle=_load(prefix+"append_only_lifecycle_v1",semantic_root/"append_only_lifecycle_v1.py")
    return dfa.StagePConstraintStateV1,projector.StagePTokenTrieProjectorV1,controller.StagePCallbackControllerV1,lifecycle.AppendOnlyLifecycleV1


def run(request_path,response_path,prompt_path,lifecycle_root):
    State,Trie,Controller,Lifecycle=_types();events=Lifecycle(lifecycle_root,actor="runner");events.emit("RUNNER_STARTED")
    try:
        ...
        controller=Controller(projector=projector)
        ...
        def allowed(_batch_id,input_ids):
            ...
            generated_ids=input_ids[prompt_tokens:].tolist()
            receipt=controller.allowed(generated_ids,lambda ids:tokenizer.decode(ids,skip_special_tokens=True));now=time.monotonic()
            if len(generated_ids)-last_heartbeat_tokens>=16 or now-heartbeat_at>=10:
                ...
                events.emit("GENERATION_HEARTBEAT",generated_tokens=len(generated_ids),decoded_utf8_bytes=len(decoded.encode()),
                    decoded_sha256=hashlib.sha256(decoded.encode()).hexdigest(),partial_output=decoded,dfa_mode=receipt.dfa_mode,
                    entry_count=receipt.entry_count,trie_cache_size=receipt.trie_cache_size,allowed_token_count=len(receipt.allowed_token_ids),
                    tracking_path=receipt.tracking_path,tracker_rebuilds=receipt.tracker_rebuilds,
                    tracker_incremental_steps=receipt.tracker_incremental_steps,generation_elapsed_seconds=round(now-generation_started,6))
                last_heartbeat_tokens=len(generated_ids);heartbeat_at=now
            return list(receipt.allowed_token_ids)
