...
def validate_deployment(v:Mapping[str,Any],g:Mapping[str,Any])->None:
 ...
 for field in ("workflow_blob_sha256","dependency_lock_root","acquisition_module_sha256","metadata_proof_module_sha256","provenance_module_sha256","trusted_root_snapshot_sha256","offline_verifier_sha256","offline_launcher_sha256","rekor_log_id"):
...
def validate_verified_attestation(s:Mapping[str,Any],*,bundle:bytes,capture_files:Mapping[str,bytes],governance:Mapping[str,Any],deployment:Mapping[str,Any])->None:
 ...
 initiation={"deployment_identity":deployment["deployment_identity"],"repository_id":deployment["repository_id"],"workflow_commit":deployment["workflow_commit"],"workflow_path":deployment["workflow_path"],"event_name":"push","run_id":s["run_id"],"run_attempt":1,"external_parameters":{}}
...
def verify_attestation_bundle(*,bundle:bytes,trusted_root:bytes,capture_files:Mapping[str,bytes],governance:Mapping[str,Any],deployment:Mapping[str,Any],verifier:Path,launcher:Path)->Mapping[str,Any]:
 """Invoke the pinned verifier offline; caller assertions can never replace cryptography."""
 validate_deployment(deployment,governance)
 if sha(Path(verifier).read_bytes())!=deployment["offline_verifier_sha256"] or sha(Path(launcher).read_bytes())!=deployment["offline_launcher_sha256"] or sha(trusted_root)!=deployment["trusted_root_snapshot_sha256"]:raise ValueError("offline launcher/verifier/trusted root pin")
 with tempfile.TemporaryDirectory() as folder:
  root=Path(folder);bundle_path=root/"bundle.json";trust_path=root/"trusted-root.json"
  bundle_path.write_bytes(bundle);trust_path.write_bytes(trusted_root)
  run=subprocess.run([str(launcher),"--deny-network","--",str(verifier),"verify-pastila-bundle","--offline","--bundle",str(bundle_path),"--trusted-root",str(trust_path),"--output","json"],capture_output=True,timeout=60,check=False,env={"NO_PROXY":"*","no_proxy":"*"})
 if run.returncode or run.stderr or not run.stdout:raise ValueError("offline attestation verification failed")
 try:summary=json.loads(run.stdout.decode("utf-8"))
 except (UnicodeDecodeError,json.JSONDecodeError) as exc:raise ValueError("offline verifier output invalid") from exc
 if not isinstance(summary,dict):raise ValueError("offline verifier output schema")
 validate_verified_attestation(summary,bundle=bundle,capture_files=capture_files,governance=governance,deployment=deployment)
 return summary
