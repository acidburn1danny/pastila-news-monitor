"""Durable executor bound to the baseline-language diagnostic runner."""
...
RUNNER_RELATIVE = Path("src/pastila_scout/experimental_core_v1_2_stage_p_scope_graph_runner_v1_3.py")
RUNNER_SHA256 = "8d5e78da8ad7b3cdbddd3c53f018a3a2c0562f7ebce4389bb5cee5215c4a3e6f"
...
DEPENDENCY_IDENTITIES = {
    ...
    Path("src/pastila_scout/semantic_admission_v2/stage_p_scope_graph_diagnostic_callback_controller_v1.py"): "ab8c6a700e96d425065c4494cd04b8ac11fe34ec6839944f7ed3b0845bf92e1c",
...
class DurableScopeGraphStagePExecutorV1_3(ExperimentalCoreV12Executor):
    def __init__(self, *, project_root: Path, durable_lifecycle_root: Path, max_output_tokens: int = 1600) -> None:
        ...
        for relative, expected in {RUNNER_RELATIVE: RUNNER_SHA256, **DEPENDENCY_IDENTITIES}.items():
            ...
            if not target.is_file() or hashlib.sha256(target.read_bytes()).hexdigest() != expected:
                raise RuntimeError(f"diagnostic adapter identity drift: {relative.as_posix()}")
    ...
    def _invoke(self, authority, trace_path: Path, trace: dict[str, object]) -> dict[str, object]:
        ...
        with tempfile.TemporaryDirectory(prefix="pastila-stage-p-diagnostic-") as directory:
            ...
            if process.returncode != 0 or not response_path.is_file():
                ...
                self._reconcile(trace_path, trace, lifecycle_root, digest, dependencies)
                raise RuntimeError("durable diagnostic runner failed")
            self._reconcile(trace_path, trace, lifecycle_root, digest, dependencies)
            ...
            if set(result) != {"output", "terminal_eos", "constraint_active"} or result["constraint_active"] is not True or not result["output"]:
                raise ValueError("durable diagnostic response invalid")
