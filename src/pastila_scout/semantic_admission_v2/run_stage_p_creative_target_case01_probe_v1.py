"""Prepared exclusive Creative Target Case 01 probe; construction does not execute it."""
...
from .stage_p_creative_target_contract_v1 import CreativeTargetLedgerV1, validate_creative_target_sources
from .stage_p_creative_target_durable_executor_v1 import DurableCreativeTargetStagePExecutorV1
from .stage_p_creative_target_evaluator_v1 import StagePCreativeTargetEvaluatorV1
from .stage_p_phase_receipt_v2 import (
    EvidencePhaseStatusV2, StagePPhaseReceiptV2, classify_persisted_stage_p_v2, persist_phase_receipt_v2,
)
...
EVALUATOR_BINDING_IDENTITY = "a6bba6cf229e5cf75352c2a0333b186841528e767e9a6638f7b7bef1e11e6ace"
EVALUATOR_IDENTITY = "9f8f441ffc01cd964e562c3aef64ccaa91d8f1c091d01e6296dd4d2a7dc58c03"
...
def construct(*, project_root: Path, evidence_root: Path):
    ...
    evaluator = StagePCreativeTargetEvaluatorV1(
        project_root=project_root, executor=executor, timeout_seconds=240.0)
    if evaluator.evaluator_identity != EVALUATOR_IDENTITY: raise RuntimeError("evaluator identity drift")
    binding = {"case_id": CASE_ID, "pack_sha256": PACK_SHA256,
        ...
        "evaluator_binding_identity": EVALUATOR_BINDING_IDENTITY, "evaluator_identity": EVALUATOR_IDENTITY,
        "request_candidate_identity": evaluator.candidate_identity, "prompt_identity": evaluator.prompt_identity,
        "schema_identity": evaluator.schema_identity, "constraint_identity": evaluator.constraint_identity,
        "grammar_identity": evaluator.grammar_identity, "tokenizer_identity": evaluator.tokenizer_identity,
        "model_identity": evaluator.model_identity, "maximum_provider_calls": 1, "retry_count": 0,
        "repair_count": 0, "selection_count": 0,
...
def _execute_once(*, evaluator, request: dict[str, object], raw_path: Path, failure_path: Path):
    try:
        ...
        return classify_persisted_stage_p_v2(raw_path=None, provider_called=True, transport_succeeded=False,
            schema_validator=lambda data: CreativeTargetLedgerV1.model_validate_json(data, strict=True),
            membership_validator=lambda ledger: validate_creative_target_sources(
    ...
    try:
        ...
        return classify_persisted_stage_p_v2(raw_path=None, provider_called=True, transport_succeeded=True,
            schema_validator=lambda item: CreativeTargetLedgerV1.model_validate_json(item, strict=True),
            membership_validator=lambda ledger: validate_creative_target_sources(
                ledger, factual_summary=request["factual_summary"], candidate=request["candidate"]))
...
def run(*, project_root: Path, evidence_root: Path):
    ...
    receipt = _execute_once(evaluator=evaluator, request=request, raw_path=evidence_root / "stage-p-raw.bin",
