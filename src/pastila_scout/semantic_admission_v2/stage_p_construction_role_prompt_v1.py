"""Construction-only prompt contract for Construction Role Audit V1."""
...
PROMPT_RELATIVE = Path("docs/artifacts/semantic-admission-v2-stage-p-construction-role-prompt-v1.txt")


class StagePConstructionRolePromptContractV1:
    def __init__(self, project_root: Path) -> None:
        data = (project_root.resolve(strict=True) / PROMPT_RELATIVE).read_bytes()
        if not data.endswith(b"\n") or data.endswith(b"\n\n"):
            raise RuntimeError("construction-role prompt padding drift")
        execution = data[:-1]
        ...
        if self.template.count("{candidate}") != 1 or self.template.count("{factual_summary}") != 1:
            raise RuntimeError("construction-role prompt placeholder drift")
        if self.template.index("{candidate}") >= self.template.index("{factual_summary}"):
            raise RuntimeError("construction-role candidate-first ordering drift")
    ...
    def render(self, *, factual_summary: str, candidate: str) -> str:
        if type(factual_summary) is not str or type(candidate) is not str or not factual_summary or not candidate:
            raise ValueError("construction-role prompt source invalid")
        if "{factual_summary}" in factual_summary or "{candidate}" in candidate:
            raise ValueError("construction-role reserved placeholder in source")
        ...
        if "{candidate}" in rendered or "{factual_summary}" in rendered:
            raise ValueError("construction-role prompt render incomplete")
        return rendered


__all__ = ("PROMPT_RELATIVE", "StagePConstructionRolePromptContractV1")
