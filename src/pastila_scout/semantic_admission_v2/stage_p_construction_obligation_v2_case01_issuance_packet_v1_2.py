"""Zero-execution materialization of the current Case 01 V1.2 issuance packet."""
...
from .stage_p_construction_obligation_v2_generation_authority_preload_v1_2 import (
...
from .stage_p_construction_obligation_v2_generation_wsl_invocation_binding_v1_2 import (
    RUNNER_MODULE,
)
...
REQUESTED_AT = datetime(2026, 8, 28, 18, 0, tzinfo=UTC)
PACKET_RELATIVE = Path(
    "docs/artifacts/semantic-admission-v2-stage-p-construction-obligation-v2-"
    "case01-successor-issuance-packet-v1-2")
EVIDENCE_RELATIVE = Path(
    ".semantic-admission-v2-stage-p-construction-obligation-v2-case01-successor-v1-2-evidence")
...
def materialize_case01_issuance_packet_v1_2(
    *, project_root: Path,
    ...
    invocation = WslExecutionBoundaryV1_1(
        canonical_model_profile_v1(with_pydantic_bridge=True)).build_invocation(
            consumer_id="construction-obligation-v2-generation-v1-2",
    ...
    application_value = {
        ...
        "schema_version": "1.2.0", "case_id": CASE_ID,
    ...
    static_value = {
        key: value for key, value in asdict(static_executor).items()
        if key != "_wsl_boundary"
    }
    candidate_value = {
        "schema_name": "pastila-construction-obligation-v2-generation-authority-candidate",
        "schema_version": "1.2.0", "receipt_status": "UNISSUED",
    ...
    rendered_value = {
        ...
        "rendered_request_identity": rendered.request_identity,
    ...
    files = {
        ...
        "rendered-prompt.json": _canonical(rendered_value),
        "host-payload.json": host_raw,
        "static-executor-binding.json": _canonical(static_value),
        "runner-request.json": runner_raw,
        "authority-receipt-candidate.json": _canonical(candidate_value),
    }
...
def _authority_body(*, host_sha: str, runner_sha: str,
                    provider_request_id: str) -> dict[str, object]:
    return {
