"""Evaluation-only Scope Graph V1.1 controller with auditable liveness failure."""
from __future__ import annotations

from collections.abc import Callable, Sequence

from .stage_p_liveness_trie_projector_v1 import (
...
class StagePScopeGraphLivenessCallbackControllerV1:
    def __init__(self, *, projector: StagePLivenessTokenTrieProjectorV1) -> None:
        self.projector = projector
        self.tracker = StagePScopeGraphIncrementalTrackerV1_1()

    def allowed(
        self, token_ids: Sequence[int], decode: Callable[[Sequence[int]], str]
    ) -> ScopeGraphCallbackReceiptV1_1:
        ...
        return ScopeGraphCallbackReceiptV1_1(
            allowed,
            prefix.path,
            prefix.suffix_characters,
            len(prefix.decoded),
            prefix.state.mode,
            prefix.state.entry_count,
            self.tracker.rebuild_steps,
            self.tracker.incremental_steps,
            self.projector.cache_size,
        )


__all__ = ("StagePScopeGraphLivenessCallbackControllerV1",)
