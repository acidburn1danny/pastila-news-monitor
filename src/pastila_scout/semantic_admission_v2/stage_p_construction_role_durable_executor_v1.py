"""Durable evaluation-only executor bound to Construction Role runner V1."""
...
RUNNER_RELATIVE = Path("src/pastila_scout/experimental_core_v1_2_stage_p_construction_role_runner_v1.py")
RUNNER_SHA256 = "225168f07082edb9df6a841c9fc50bea8e1b92fa7d106242bc4986e83736a718"
...
DEPENDENCY_IDENTITIES = {
    ...
    Path("src/pastila_scout/semantic_admission_v2/stage_p_construction_role_constraint_v1.py"): "a920d2e2c659246791ef250fb49d7d9a28c504b331224c6006df2b8f6866aac9",
    ...
    Path("src/pastila_scout/semantic_admission_v2/stage_p_construction_role_incremental_tracker_v1.py"): "3ef75f2fd7178df4c68dc81388605bec6152e5bf7a215a82772d3c7e2aa87a03",
    Path("src/pastila_scout/semantic_admission_v2/stage_p_construction_role_callback_controller_v1.py"): "ee4fe113dfa01f71aef6bf99c635059b8ac60bc14c3516b96803b2feb3921347",
...
class DurableConstructionRoleStagePExecutorV1(ExperimentalCoreV12Executor):
    def __init__(self, *, project_root: Path, durable_lifecycle_root: Path,
        ...
        for relative, expected in {RUNNER_RELATIVE: RUNNER_SHA256, **DEPENDENCY_IDENTITIES}.items():
            ...
            if not target.is_file() or hashlib.sha256(target.read_bytes()).hexdigest() != expected:
                raise RuntimeError(f"construction-role dependency identity drift: {relative.as_posix()}")
    ...
    def _invoke(self, authority, trace_path: Path, trace: dict[str, object]) -> dict[str, object]:
        ...
        with tempfile.TemporaryDirectory(prefix="pastila-stage-p-construction-role-") as directory:
            ...
            try:
                ...
                try:
                    stdout, stderr = process.communicate(timeout=5); termination = "TERMINATED"
                except subprocess.TimeoutExpired:
            ...
            if process.returncode != 0 or not response_path.is_file():
                ...
                self._reconcile(trace_path, trace, lifecycle_root, digest, dependencies)
                raise RuntimeError("durable construction-role runner failed")
            self._reconcile(trace_path, trace, lifecycle_root, digest, dependencies)
            ...
            if (set(result) != {"output", "terminal_eos", "constraint_active"}
                ...
                raise ValueError("durable construction-role response invalid")
...
__all__ = ("DEPENDENCY_IDENTITIES", "RUNNER_RELATIVE", "RUNNER_SHA256", "VENV_PYTHON",
           "WSL_DISTRIBUTION", "DurableConstructionRoleStagePExecutorV1")
