"""Durable Track-A executor with distinct constraint-liveness classification."""
...
RUNNER_RELATIVE = Path("src/pastila_scout/experimental_core_v1_2_stage_p_scope_graph_runner_v1_2.py")
RUNNER_SHA256 = "ac25019c195329064fdfadc3b41b3f19b17d531aebb0ebbd06486f2c9a94c42e"
...
DEPENDENCY_IDENTITIES = {
    ...
    Path("src/pastila_scout/semantic_admission_v2/stage_p_scope_graph_incremental_tracker_v1_2.py"): "5160c7a01b86c7bcddf18ca0b6999fde696fefaebf92ce0c958d5b535a865c84",
    Path("src/pastila_scout/semantic_admission_v2/stage_p_scope_graph_liveness_callback_controller_v1_2.py"): "e165d1da672ce96812fe7e01d0d685d9479916065cb8ccaea6b08636d8e26681",
...
class ConstraintLivenessExecutionReceiptV1:
    code: str
...
class StagePConstraintLivenessExecutionErrorV1(RuntimeError):
    def __init__(self, receipt: ConstraintLivenessExecutionReceiptV1) -> None:
        super().__init__(receipt.code)
        self.receipt = receipt


def read_constraint_liveness_failure_v1(root: Path) -> ConstraintLivenessExecutionReceiptV1 | None:
    for path in sorted(root.glob("runner-*-runner-exception.json"), reverse=True):
        value = json.loads(path.read_bytes())
        if value.get("exception_type") != "StagePConstraintLivenessErrorV1":
            continue
        detail = json.loads(value.get("exception_message", ""))
        required = {"code", "decoded_utf8_bytes", "decoded_sha256", "dfa_mode", "dfa_next_step", "entry_count"}
        if set(detail) != required or detail["code"] != "CONSTRAINT_LIVENESS_FAILURE":
            raise ValueError("CONSTRAINT_LIVENESS_RECEIPT_INVALID")
        if type(detail["decoded_utf8_bytes"]) is not int or detail["decoded_utf8_bytes"] < 0:
            raise ValueError("CONSTRAINT_LIVENESS_RECEIPT_INVALID")
        if type(detail["decoded_sha256"]) is not str or len(detail["decoded_sha256"]) != 64:
            raise ValueError("CONSTRAINT_LIVENESS_RECEIPT_INVALID")
        return ConstraintLivenessExecutionReceiptV1(**detail)
    return None


class DurableScopeGraphStagePExecutorV1_2(ExperimentalCoreV12Executor):
    def __init__(self, *, project_root: Path, durable_lifecycle_root: Path, max_output_tokens: int = 1600) -> None:
        ...
        for relative, expected in {RUNNER_RELATIVE: RUNNER_SHA256, **DEPENDENCY_IDENTITIES}.items():
            ...
            if not target.is_file() or hashlib.sha256(target.read_bytes()).hexdigest() != expected:
                raise RuntimeError(f"scope-graph Track-A identity drift: {relative.as_posix()}")
    ...
    def _invoke(self, authority, trace_path: Path, trace: dict[str, object]) -> dict[str, object]:
        ...
        lifecycle_root = self._durable_root / digest
        ...
        with tempfile.TemporaryDirectory(prefix="pastila-stage-p-scope-graph-track-a-") as directory:
            root = Path(directory)
            ...
            trace.update(runner_launch_attempted=True, runner_path=_wsl_path(runner),
                         durable_lifecycle_relative_path=digest)
            ...
            if process.returncode != 0 or not response_path.is_file():
                ...
                if liveness is not None:
                    value = liveness.as_json_value()
                    events.emit("HOST_CONSTRAINT_LIVENESS_FAILURE_CLASSIFIED", **value)
                ...
                self._reconcile(trace_path, trace, lifecycle_root, digest, dependencies)
                raise RuntimeError("durable scope-graph Track-A runner failed")
            self._reconcile(trace_path, trace, lifecycle_root, digest, dependencies)
            ...
            if set(result) != {"output", "terminal_eos", "constraint_active"} or result["constraint_active"] is not True or not result["output"]:
                raise ValueError("durable scope-graph Track-A response invalid")
...
__all__ = ("DEPENDENCY_IDENTITIES", "RUNNER_RELATIVE", "RUNNER_SHA256", "VENV_PYTHON", "WSL_DISTRIBUTION",
           "ConstraintLivenessExecutionReceiptV1", "StagePConstraintLivenessExecutionErrorV1",
           "DurableScopeGraphStagePExecutorV1_2", "read_constraint_liveness_failure_v1")
