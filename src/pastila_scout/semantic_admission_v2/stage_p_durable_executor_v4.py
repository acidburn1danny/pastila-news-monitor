"""Evaluation-only Stage P V4 executor with authoritative lifecycle reconciliation."""
...
class DurableConstrainedStagePCoreV12ExecutorV4(ExperimentalCoreV12Executor):
    def __init__(self,*,project_root:Path,durable_lifecycle_root:Path,max_output_tokens:int=1400)->None:
        ...
        for relative,expected in {RUNNER_RELATIVE:RUNNER_SHA256,**DEPENDENCY_IDENTITIES}.items():
            ...
            if not target.is_file() or hashlib.sha256(target.read_bytes()).hexdigest()!=expected:
                raise RuntimeError(f"durable Stage P V4 identity drift: {relative.as_posix()}")
    ...
    def _invoke(self,authority,trace_path:Path,trace:dict[str,object])->dict[str,object]:
        ...
        dependencies={path.as_posix():value for path,value in DEPENDENCY_IDENTITIES.items() if path.name!="durable_lifecycle_reconciliation_v1.py"}
        payload={"prompt":"\n\n".join(message.content for message in unit.messages),"max_new_tokens":self._max_output_tokens}
        with tempfile.TemporaryDirectory(prefix="pastila-stage-p-durable-v4-") as directory:
            root=Path(directory);request_path=root/"request.json";response_path=root/"response.json"
            request_path.write_text(json.dumps(payload,ensure_ascii=False),"utf-8");runner=self._project_root/RUNNER_RELATIVE
            ...
            events.emit("HOST_LAUNCH",request_id_sha256=request_digest,request_sha256=hashlib.sha256(request_path.read_bytes()).hexdigest(),
                runner_sha256=RUNNER_SHA256,dependency_identities=dependencies,timeout_seconds=authority.timeout_policy.timeout_seconds)
            ...
            try:
                ...
                self._reconcile(trace_path,trace,lifecycle_root,request_digest,dependencies);raise
            ...
            if process.returncode!=0 or not response_path.is_file(): raise RuntimeError("durable Stage P V4 runner failed")
            ...
            if set(result)!={"output","terminal_eos","constraint_active"} or result["constraint_active"] is not True or not result["output"]:
                raise ValueError("durable Stage P V4 response invalid")
    ...
    def _reconcile(self,trace_path,trace,lifecycle_root,relative_path,dependencies):
...
__all__=("DurableConstrainedStagePCoreV12ExecutorV4",)
