"""Construction-only contract for the Stage P Scope Graph V1.1 prompt candidate."""
...
PROMPT_RELATIVE = Path("docs/artifacts/semantic-admission-v2-stage-p-scope-graph-prompt-v1-1.txt")


class StagePScopeGraphPromptContractV1_1:
    def __init__(self, project_root: Path) -> None:
        data = (project_root.resolve(strict=True) / PROMPT_RELATIVE).read_bytes()
        if not data.endswith(b"\n") or data.endswith(b"\n\n"):
            raise RuntimeError("Stage P scope-graph V1.1 prompt padding drift")
        execution = data[:-1]
    ...
    def render(self, *, factual_summary: str, candidate: str) -> str:
        if type(factual_summary) is not str or type(candidate) is not str or not factual_summary or not candidate:
            raise ValueError("Stage P scope-graph V1.1 source text invalid")
        if "{factual_summary}" in factual_summary or "{candidate}" in candidate:
            raise ValueError("Stage P scope-graph V1.1 reserved placeholder in source text")
        ...
        if "{factual_summary}" in rendered or "{candidate}" in rendered:
            raise ValueError("Stage P scope-graph V1.1 render incomplete")
        return rendered


__all__ = ("PROMPT_RELATIVE", "StagePScopeGraphPromptContractV1_1")
