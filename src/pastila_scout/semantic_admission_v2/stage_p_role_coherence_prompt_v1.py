"""Construction-only contract for the Stage P role-coherence prompt candidate."""
...
PROMPT_RELATIVE = Path("docs/artifacts/semantic-admission-v2-stage-p-role-coherence-prompt-v1.txt")


class StagePRoleCoherencePromptContractV1:
    def __init__(self, project_root: Path) -> None:
        data = (project_root.resolve(strict=True) / PROMPT_RELATIVE).read_bytes()
        if not data.endswith(b"\n") or data.endswith(b"\n\n"):
            raise RuntimeError("Stage P role-coherence prompt padding drift")
        execution = data[:-1]
    ...
    def render(self, *, factual_summary: str, candidate: str) -> str:
        ...
        if "{factual_summary}" in rendered or "{candidate}" in rendered:
            raise ValueError("Stage P role-coherence render incomplete")
        return rendered


__all__ = ("PROMPT_RELATIVE", "StagePRoleCoherencePromptContractV1")
