"""Trie callback controller bound to the approved Stage P scope-graph tracker."""
from __future__ import annotations

from collections.abc import Callable, Sequence
from dataclasses import dataclass

from .stage_p_scope_graph_incremental_tracker_v1 import StagePScopeGraphIncrementalTrackerV1
from .stage_p_trie_projector_v1 import StagePTokenTrieProjectorV1


@dataclass(frozen=True)
class ScopeGraphCallbackReceiptV1:
...
class StagePScopeGraphCallbackControllerV1:
    def __init__(self, *, projector: StagePTokenTrieProjectorV1) -> None:
        self.projector = projector
        self.tracker = StagePScopeGraphIncrementalTrackerV1()

    def allowed(self, token_ids: Sequence[int], decode: Callable[[Sequence[int]], str]) -> ScopeGraphCallbackReceiptV1:
        prefix = self.tracker.state_for(token_ids, decode)
        allowed = self.projector.allowed_token_ids(prefix.state)
        return ScopeGraphCallbackReceiptV1(
...
__all__ = ("ScopeGraphCallbackReceiptV1", "StagePScopeGraphCallbackControllerV1")
