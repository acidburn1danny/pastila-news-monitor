"""V13 identities over byte-pinned candidate execution validation mechanics."""
...
_changes = {
    ...
    "scripts/execute_production_core_candidate_qualification_v3.py": "scripts/execute_production_core_candidate_qualification_v13.py",
    "scripts/launch_production_core_candidate_qualification_v9.py": "scripts/launch_production_core_candidate_qualification_v13.py",
    "scripts/run_production_core_candidate_qualification_v3.sh": "scripts/run_production_core_candidate_qualification_v11.sh",
    "src/pastila_scout/production_core_candidate_execution_authority_v3.py": "src/pastila_scout/production_core_candidate_execution_authority_v13.py",
    "src/pastila_scout/production_core_candidate_qualification_runner_v3.py": "src/pastila_scout/production_core_candidate_qualification_runner_v12.py",
}
for _old, _new in _changes.items():
    ...
    if _source.count(_old) != _count:
        raise RuntimeError(f"V13 validator projection witness mismatch: {_old}")
    _source = _source.replace(_old, _new)
_syntax_witness = "except TypeError, ValueError:"
if _source.count(_syntax_witness) != 1:
    raise RuntimeError("V13 validator syntax repair witness mismatch")
...
if _source.count(_rootfs_witness) != 1:
    raise RuntimeError("V13 shared-rootfs validator witness mismatch")
