"""WSL-side single-request runner for the frozen experimental Core V1.2."""
...
def main() -> None:
    request_path, response_path, prompt_path, lifecycle_path = map(Path, sys.argv[1:5])
    lifecycle = {
    ...
    lifecycle["model_load_started"] = True
    _write_lifecycle(lifecycle_path, lifecycle)
    ...
    model.eval()
    lifecycle["model_load_succeeded"] = True
    _write_lifecycle(lifecycle_path, lifecycle)
    ...
    lifecycle["inference_started"] = True
    _write_lifecycle(lifecycle_path, lifecycle)
    ...
    lifecycle["inference_succeeded"] = True
    _write_lifecycle(lifecycle_path, lifecycle)
...
def _write_lifecycle(path: Path, value: dict[str, bool]) -> None:
