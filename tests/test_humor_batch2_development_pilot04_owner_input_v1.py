...
def test_pilot04_request_is_content_free_independent_and_fail_closed() -> None:
    value = load("humor-mechanics-batch2-development-pilot04-owner-input-request-v1.json")
    identity = value.pop("owner_input_request_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT04_OWNER_INPUT_REQUEST_V1", value)
    ...
    assert value["pool_status_preserved"] == "POOL_PENDING_INSUFFICIENT_INDEPENDENT_POSITIVE_FAMILIES"
...
def test_pilot04_template_requires_owner_choices_and_prior_pilot_independence() -> None:
    value = load("humor-mechanics-batch2-development-pilot04-owner-declaration-template-v1.json")
    identity = value.pop("template_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT04_OWNER_DECLARATION_TEMPLATE_V1", value)
    assert value["pilot_id"] == "BATCH2-INTERNALLY-OWNED-DEVELOPMENT-PILOT-04"
    assert all(item == "OWNER_MUST_CHOOSE_BOOLEAN_INDEPENDENTLY" for item in value["independent_grants"].values())
    assert "pilots_01_02_03" in " ".join(value["source_status_declarations"]).lower()


def test_pilot04_audit_stops_for_owner_input_without_g04b() -> None:
    value = load("humor-mechanics-batch2-development-pilot04-owner-input-request-audit-v1.json")
    identity = value.pop("audit_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT04_OWNER_INPUT_REQUEST_AUDIT_V1", value)
