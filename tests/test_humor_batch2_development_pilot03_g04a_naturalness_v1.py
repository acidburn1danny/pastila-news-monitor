...
def test_blind_naturalness_review_passes_with_only_nonmaterial_reservation() -> None:
    value = load("humor-mechanics-batch2-development-pilot03-candidate01-g04a-naturalness-review-v1.json")
    identity = value.pop("g04a_review_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT03_CANDIDATE01_G04A_NATURALNESS_REVIEW_V1", value)
    ...
    assert value["reservations"] == ["MINOR_ABSTRACT_NOMINALIZATION_NONMATERIAL"]
...
def test_naturalness_receipt_is_sealed_and_voice_remains_separate() -> None:
    value = load("humor-mechanics-batch2-development-pilot03-candidate01-g04a-naturalness-receipt-v1.json")
    identity = value.pop("g04a_receipt_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT03_CANDIDATE01_G04A_NATURALNESS_RECEIPT_V1", value)
    assert value["g04a_verdict"] == "ROMANIAN_NATURALNESS_PASS"
