...
def test_worker_real_branch_is_beyond_gate(monkeypatch):
 ...
 with pytest.raises(RuntimeError,match="not authorized"): m.run_slot(*([Path("x")]*7),"T0_CONTROL_S0_CONTROL",161803)
def test_real_supervisor_and_route_require_separate_owner_authority(monkeypatch):
 ...
 assert "learning-signal.jsonl" in text and "measurement-annotations.jsonl" in text
