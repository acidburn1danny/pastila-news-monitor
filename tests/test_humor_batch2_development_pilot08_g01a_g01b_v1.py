"""Verify frozen Pilot 08 G01A/G01B admission artifacts."""
...
def test_pilot08_g01a_g01b_are_sealed_and_sufficiency_is_deferred() -> None:
    receipt = json.loads((ART / "humor-mechanics-batch2-development-pilot08-g01a-g01b-admission-v1.json").read_text(encoding="utf-8"))
    audit = json.loads((ART / "humor-mechanics-batch2-development-pilot08-g01a-g01b-admission-v1-audit.json").read_text(encoding="utf-8"))
    core = dict(receipt); identity = core.pop("admission_identity")
    assert seal("B2_DEVELOPMENT_PILOT08_G01A_G01B_ADMISSION_V1", core) == identity
    ...
    assert seal("B2_DEVELOPMENT_PILOT08_G01A_G01B_ADMISSION_AUDIT_V1", core) == identity
