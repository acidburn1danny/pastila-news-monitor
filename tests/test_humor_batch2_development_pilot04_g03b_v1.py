...
def test_g03b_confirms_causality_without_materializing_counterfactuals() -> None:
    review = json.loads((ART / "humor-mechanics-batch2-development-pilot04-candidate01-g03b-review-v1.json").read_text(encoding="utf-8"))
    identity = review.pop("g03b_review_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT04_CANDIDATE01_G03B_REVIEW_V1", review)
...
def test_g03b_receipt_is_sealed_and_non_authorizing() -> None:
    receipt = json.loads((ART / "humor-mechanics-batch2-development-pilot04-candidate01-g03b-receipt-v1.json").read_text(encoding="utf-8"))
    identity = receipt.pop("g03b_receipt_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT04_CANDIDATE01_G03B_RECEIPT_V1", receipt)
