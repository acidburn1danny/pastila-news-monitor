...
RUNNER = ROOT / "scripts/evaluate_editor_core_v10_v12_targeted_holdout_r5.py"
AUDITOR = ROOT / "scripts/audit_editor_core_v10_v12_targeted_holdout_r5.py"
SHELL = ROOT / "scripts/run_editor_core_v10_v12_targeted_holdout_r5.sh"
...
def test_requests_are_complete_and_target_free():
    runner = load(RUNNER, "r5_runner")
    rows = runner.validate_requests(ROOT / "docs/artifacts/editor-core-v10-v12-targeted-continuation-r5-holdout-requests.jsonl")
    assert len({runner.request_identity(row) for row in rows}) == 12


def test_fixture_smoke_and_leakage_rejection():
    runner = load(RUNNER, "r5_smoke")
...
def test_auditor_closes_receipt_rows_and_classes(tmp_path):
    auditor = load(AUDITOR, "r5_auditor")
...
def test_semantic_audit_detects_epistemic_regression():
    auditor = load(AUDITOR, "r5_semantic")
    key = {"diagnostic_pattern": "ATTRIBUTION_DENIAL_FINALITY"}
    safe = {"text": "Comisia suspectează că operatorul ar fi greșit; acesta neagă acuzația. Inspectoratul nu a confirmat abaterea, iar procedura continuă."}
    asserted = {"text": "Comisia a confirmat că operatorul a greșit; acesta neagă acuzația. Procedura s-a încheiat."}
...
def test_auditor_rejects_extra_output(tmp_path):
    ...
    with pytest.raises(ValueError, match="closure"):
        load(AUDITOR, "r5_extra").audit(output, tmp_path / "key", "x", "a", "q", "r")
...
def test_shell_binds_only_the_two_expected_adapters_and_excludes_answer_key():
    ...
    assert "r5-step-6" in source and "41e280f26688e3cc1afdf94622a16ff238fdca8c600c930105ed008c6cb43024" in source
    assert "EXPECTED_PARENT_COMMIT=3e285af538157a0ca0dc63e149aa1330adf5ac39" in source
