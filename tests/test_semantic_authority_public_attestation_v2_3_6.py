...
def test_pinned_offline_verifier_is_executed(monkeypatch,tmp_path):
 ...
 files={"capture/a":b"x"};s=attestation(g,d,files);monkeypatch.setattr(m.subprocess,"run",lambda *a,**k:SimpleNamespace(returncode=0,stderr=b"",stdout=m.canonical(s)))
 assert m.verify_attestation_bundle(bundle=b"offline bundle",trusted_root=trusted,capture_files=files,governance=g,deployment=d,verifier=verifier,launcher=launcher)==s
 with pytest.raises(ValueError,match="pin"):m.verify_attestation_bundle(bundle=b"offline bundle",trusted_root=b"wrong",capture_files=files,governance=g,deployment=d,verifier=verifier,launcher=launcher)
