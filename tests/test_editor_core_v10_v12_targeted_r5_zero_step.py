...
LAUNCHER = ROOT / "scripts/launch_editor_core_v10_v12_targeted_r5_zero_step.py"


def module():
    spec = importlib.util.spec_from_file_location("targeted_r5_zero_step", LAUNCHER)
...
def test_publication_selection_and_r5_dataset_close():
    m = module()
    manifest, config, audit = m.validate_public_inputs()
    assert audit["verdict"] == "PASS" and audit["blockers"] == 0
    assert manifest["manifest_identity"] == "1cb2afd04273089c6f7943369c9b158182c5efdccf003f45f85b87fd2881f769"
    assert config["training_config_identity"] == "1781cc93fb9de7c5d88a37af8286f36874beb646df5c3b12383e7b4eb2c55e34"
...
def test_published_r5_blob_drift_fails_closed(monkeypatch):
    m = module()
    real_read_bytes = Path.read_bytes

    def read_bytes(path):
        if path.name == "editor-core-v10-v12-targeted-continuation-r5-manifest.json":
            return b"{}"
        return real_read_bytes(path)
