...
def test_g05_request_requires_explicit_owner_decision_and_preserves_limits() -> None:
    value = load("humor-mechanics-batch2-development-pilot04-candidate01-g05-owner-review-request-v1.json")
    identity = value.pop("g05_owner_review_request_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT04_CANDIDATE01_G05_OWNER_REVIEW_REQUEST_V1", value)
    ...
    assert value["bound_gate_results"]["g03c_pool"] == "POOL_PENDING_PILOT04_QUALITY_GATES_AND_SEPARATE_G04B_CERTIFICATION"
...
def test_g05_request_audit_is_sealed_and_does_not_infer_approval() -> None:
    value = load("humor-mechanics-batch2-development-pilot04-candidate01-g05-owner-review-request-audit-v1.json")
    identity = value.pop("audit_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT04_CANDIDATE01_G05_OWNER_REVIEW_REQUEST_AUDIT_V1", value)
