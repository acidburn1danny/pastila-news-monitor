...
LAUNCHER = ROOT / "scripts/launch_editor_core_v10_v12_targeted_r3_zero_step.py"


def module():
    spec = importlib.util.spec_from_file_location("targeted_r3_zero_step", LAUNCHER)
...
def test_publication_selection_and_r3_dataset_close():
    m = module()
    manifest, config, audit = m.validate_public_inputs()
    assert audit["verdict"] == "PASS" and audit["blockers"] == 0
    assert manifest["manifest_identity"] == "961f7ce0964032023c9c3963ab3ef18b614fe9b7b0fa33231c30deab38b97ee2"
    assert config["training_config_identity"] == "c5c91295e19865b4a063570cf7a2186f1a8cd10dbe61da7618aab435d8a80db7"
...
def test_semantic_selection_substitution_fails(monkeypatch, field):
    m = module()
    original = json.loads((m.ARTIFACTS / "editor-core-v10-v12-targeted-r2-semantic-result.json").read_bytes())
    if field == "adapter":
        original["selection"]["selected_adapter_identity"] = "0" * 64
    elif field == "checkpoint":
        original["r2"]["checkpoint_identity"] = "0" * 64
    else:
    ...
    def read_bytes(path):
        if path.name == "editor-core-v10-v12-targeted-r2-semantic-result.json":
            return replacement
