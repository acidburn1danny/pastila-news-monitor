...
def test_blind_voice_review_passes_without_using_mechanism_or_owner_preferences() -> None:
    value = load("humor-mechanics-batch2-development-pilot03-candidate01-voice-review-v1.json")
    identity = value.pop("voice_review_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT03_CANDIDATE01_BLIND_VOICE_REVIEW_V1", value)
...
def test_voice_receipt_is_sealed_and_owner_review_remains_separate() -> None:
    value = load("humor-mechanics-batch2-development-pilot03-candidate01-voice-receipt-v1.json")
    identity = value.pop("voice_receipt_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT03_CANDIDATE01_BLIND_VOICE_RECEIPT_V1", value)
    assert value["voice_verdict"] == "VOICE_PASS"
