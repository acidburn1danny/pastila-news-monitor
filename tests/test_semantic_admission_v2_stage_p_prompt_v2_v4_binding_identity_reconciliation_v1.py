...
HISTORICAL = ROOT / ".semantic-admission-v2-stage-p-prompt-v2-v4-binding-evidence/manifest.json"
RECONCILIATION = ROOT / "docs/artifacts/semantic-admission-v2-stage-p-prompt-v2-v4-binding-identity-reconciliation-v1.json"
...
def test_historical_manifest_identity_and_expected_hashes_remain_immutable() -> None:
    ...
    values = list(manifest["source_identities"])
    values.extend(item["sha256"].lower() for item in manifest["artifacts"])
    identity = hashlib.sha256("\n".join(values).encode("utf-8")).hexdigest()
    assert identity == manifest["canonical_identity"] == "8066934252d94c1bf2097340798546131145c2f322a4959d2e46bccf363391f5"
...
def test_only_the_three_recorded_historical_sources_differ() -> None:
    ...
    assert mismatches == [
        "docs/artifacts/semantic-admission-v2-stage-p-prompt-v2-v4-binding-candidate.json",
        "src/pastila_scout/semantic_admission_v2/stage_p_source_role_evaluator_v2.py",
        "tests/test_semantic_admission_v2_stage_p_prompt_v2_v4_binding.py",
    ]
...
def test_current_sources_and_provenance_are_bound_without_equivalence_claim() -> None:
    ...
    for item in receipt["unavailable_historical_sources"]:
        ...
        assert item["current_provenance_commit"] == "6aa84195a876c0e891160bd009da7db43e09b07d"
