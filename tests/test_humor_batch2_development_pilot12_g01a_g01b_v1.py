...
def test_pilot12_g01_admission_is_sealed_and_non_authorizing() -> None:
    admission = json.loads((ART / "humor-mechanics-batch2-development-pilot12-g01a-g01b-admission-v1.json").read_text(encoding="utf-8"))
    audit = json.loads((ART / "humor-mechanics-batch2-development-pilot12-g01a-g01b-admission-v1-audit.json").read_text(encoding="utf-8"))
    core = dict(admission); identity = core.pop("admission_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT12_G01A_G01B_ADMISSION_V1", core)
    core = dict(audit); audit_identity = core.pop("audit_identity")
    assert audit_identity == seal("B2_DEVELOPMENT_PILOT12_G01A_G01B_ADMISSION_AUDIT_V1", core)
    assert admission["g01a"]["verdict"] == admission["g01b"]["verdict"] == "PASS"
    assert admission["g01a"]["source_git_object"] == "7cb8282216f81cf1c774eab46081c57e490529a1"
