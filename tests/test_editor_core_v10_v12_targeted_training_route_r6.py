...
SMOKE = ROOT / "scripts/smoke_editor_core_v10_v12_targeted_training_route_r6.py"
CONFIG = ROOT / "docs/artifacts/editor-core-v10-v12-targeted-continuation-r6-training-config.json"
...
def test_fixture_smoke_closes_without_ml_execution():
    result = load(SMOKE, "r6_route_smoke").run()
...
def test_worker_plan_is_exactly_36_rows_6_steps_and_final_checkpoint():
    worker = load(WORKER, "targeted_r6_worker")
...
def test_worker_rejects_material_contract_drift(mutation):
    worker = load(WORKER, "targeted_r6_worker_negative")
    config = json.loads(CONFIG.read_bytes())
    if mutation == "rows":
        with pytest.raises(ValueError, match="corpus closure"):
            worker.validate_config(config, worker.EXPECTED_CORPUS, 35)
        return
    key, value = {"optimizer": ("optimizer", "PAGED_ADAMW_8BIT"), "holdout": ("r2_r3_r4_r5_holdout_training_use", True), "parent": ("parent_checkpoint_identity", "0" * 64), "seed": ("seed", 271828)}[mutation]
    config[key] = value
...
def test_bound_route_is_isolated_and_binds_r6_sources():
    ...
    assert "EDITOR_CORE_R6_TRAINING_OWNER_AUTHORIZED" in source
    assert "EXPECTED_SOURCE_COMMIT=6494e127b84c67ca7ed2d14ec2ba95cd2195b7e1" in source
    assert "EXPECTED_SOURCE_TREE=0f9cdd4d2e8362f036c4d640ea591e637aad39d4" in source
