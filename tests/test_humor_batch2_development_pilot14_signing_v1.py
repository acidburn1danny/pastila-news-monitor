...
SCRIPT = ROOT / "scripts/owner_humor_batch2_development_pilot14_signing_v1.py"


def load_module():
    spec = importlib.util.spec_from_file_location("pilot14_signing", SCRIPT)
...
def test_frozen_packet_is_exactly_unsigned_eight_request_packet() -> None:
    ...
    assert packet["downstream_planning_or_construction_performed"] is False
...
def test_secret_and_response_paths_must_be_outside_repository() -> None:
    module = load_module()
    external = Path(f"{ROOT.drive}\\Pilot13-owner-controlled-external-test").resolve()
