...
def test_owner_freeze_binds_exact_decision_and_preserves_pool_pending() -> None:
    value = load("humor-mechanics-batch2-development-pilot03-candidate01-g05-owner-freeze-v1.json")
    identity = value.pop("g05_owner_freeze_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT03_CANDIDATE01_G05_OWNER_FREEZE_V1", value)
    assert value["owner_decision"] == "APPROVE PILOT03 OWNER_FROZEN DEVELOPMENT_ONLY POOL_PENDING"
    assert value["status"] == "OWNER_FROZEN_DEVELOPMENT_ONLY_POOL_PENDING"
...
def test_owner_freeze_audit_is_sealed_and_grants_no_downstream_authority() -> None:
    value = load("humor-mechanics-batch2-development-pilot03-candidate01-g05-owner-freeze-audit-v1.json")
    identity = value.pop("audit_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT03_CANDIDATE01_G05_OWNER_FREEZE_AUDIT_V1", value)
