...
SCRIPT = ROOT / "scripts/owner_humor_batch2_development_pilot12_signing_v1.py"


def load_module():
    spec = importlib.util.spec_from_file_location("pilot12_signing", SCRIPT)
...
def test_frozen_packet_is_exactly_unsigned_eight_request_packet() -> None:
    ...
    assert packet["realization_candidate_emission_coordinate_conformance_or_semantic_edge_validation_performed"] is False
...
def test_secret_and_response_paths_must_be_outside_repository() -> None:
    module = load_module()
    external = Path(f"{ROOT.drive}\\Pilot12-owner-controlled-external-test").resolve()
