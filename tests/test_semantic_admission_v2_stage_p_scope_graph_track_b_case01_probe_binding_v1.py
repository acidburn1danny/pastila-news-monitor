...
from pastila_scout.semantic_admission_v2.run_stage_p_scope_graph_track_b_case01_probe_v1 import (
    CASE_ID,
    EVALUATOR_BINDING_IDENTITY,
    PACK_RELATIVE,
    PACK_SHA256,
    construct,
)
...
def test_construct_binds_only_frozen_case01_without_launch(tmp_path, monkeypatch):
    calls = []
    ...
    assert CASE_ID == "HMCV1-SASC-01" and binding["case_id"] == CASE_ID
...
def test_construct_rejects_nonexclusive_or_drifted_evidence_root(tmp_path, monkeypatch):
...
def test_probe_source_has_one_call_path_and_no_stage_c_edge():
    source = (ROOT / "src/pastila_scout/semantic_admission_v2/run_stage_p_scope_graph_track_b_case01_probe_v1.py").read_text("utf-8")
