...
def test_optimized_projector_and_generated_suffix_contract_are_source_bound():
    ...
    expected = {
        ...
        "CANONICAL_SEMANTIC_COMPLETENESS_SOURCE_SHA256":
...
def test_superseded_optimized_projector_receipt_cannot_build_durable_successor(
    ...
    with pytest.raises(ValueError):
        binding.build_generation_wsl_invocation_v1_2_1(
            project_root=ROOT,
            policy_receipt_path=ROOT / "docs/artifacts/semantic-admission-v2-stage-p-"
                "construction-obligation-v2-generation-policy-validation-receipt-v1.json",
            authority_receipt_path=superseded,
            runner_request_path=packet / "runner-request.json",
            packet_manifest_path=packet / "manifest.json",
            system_prompt_path=ROOT / ".experimental-0-3-core-v1-2-journalistic-"
                "deontology-prime-directive-v1-evidence/"
