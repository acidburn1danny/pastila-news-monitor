...
from pastila_scout.semantic_admission_v2.run_stage_p_construction_obligation_case01_probe_v1 import (
    CASE_ID, DFA_CANDIDATE_IDENTITY, EVALUATOR_BINDING_IDENTITY, EVALUATOR_IDENTITY,
    PACK_RELATIVE, PACK_SHA256, construct,
)
...
def test_construct_binds_only_case01_without_launch(tmp_path, monkeypatch):
    calls = []
    def forbidden(*args, **kwargs): calls.append((args, kwargs)); raise AssertionError("execution forbidden")
    ...
    assert binding["dfa_candidate_identity"] == DFA_CANDIDATE_IDENTITY
...
def test_construct_requires_fresh_exclusive_root(tmp_path, monkeypatch):
...
def test_wrapper_has_single_call_typed_failure_and_no_stage_c():
    source = (ROOT / "src/pastila_scout/semantic_admission_v2/run_stage_p_construction_obligation_case01_probe_v1.py").read_text("utf-8")
