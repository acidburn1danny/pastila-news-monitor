...
SMOKE = ROOT / "scripts/smoke_editor_core_v10_v12_targeted_training_route_r4.py"
CONFIG = ROOT / "docs/artifacts/editor-core-v10-v12-targeted-continuation-r4-training-config.json"
...
def test_fixture_smoke_closes_without_ml_execution():
    result = load(SMOKE, "r4_route_smoke").run()
...
def test_worker_plan_is_exactly_48_rows_6_steps_and_final_checkpoint():
    worker = load(WORKER, "targeted_r4_worker")
...
def test_worker_rejects_material_contract_drift(mutation):
    worker = load(WORKER, "targeted_r4_worker_negative")
...
def test_bound_route_is_isolated_and_binds_r4_sources():
    ...
    assert "EDITOR_CORE_R4_TRAINING_OWNER_AUTHORIZED" in source
    assert "EXPECTED_SOURCE_COMMIT=4d55b41c591454305b95b2f32b4259d77b6f3db1" in source
    assert "EXPECTED_SOURCE_TREE=d950f57fe89dc39e336176f9ae105186ead757fd" in source
