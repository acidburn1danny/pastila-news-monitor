"""Verify Pilot 04 Governance V2 assignment-design artifacts."""
...
def test_pilot04_assignment_is_label_blind_source_bound_and_non_authorizing() -> None:
    mapping = json.loads((ART / "humor-mechanics-batch2-development-pilot04-sealed-assignment-mapping-v1.json").read_text(encoding="utf-8"))
    packet = json.loads((ART / "humor-mechanics-batch2-development-pilot04-constructor-facing-assignment-proposal-v1.json").read_text(encoding="utf-8"))
    audit = json.loads((ART / "humor-mechanics-batch2-development-pilot04-assignment-design-leakage-audit-v1.json").read_text(encoding="utf-8"))
    mapping_core = dict(mapping)
    mapping_id = mapping_core.pop("sealed_assignment_identity")
    assert seal("B2_DEVELOPMENT_PILOT04_SEALED_ASSIGNMENT_V1", mapping_core) == mapping_id
    packet_core = dict(packet)
    packet_id = packet_core.pop("constructor_facing_packet_identity")
    assert seal("B2_DEVELOPMENT_PILOT04_CONSTRUCTOR_PACKET_V1", packet_core) == packet_id
    ...
    assert seal("B2_DEVELOPMENT_PILOT04_ASSIGNMENT_DESIGN_AUDIT_V1", audit_core) == audit_id
