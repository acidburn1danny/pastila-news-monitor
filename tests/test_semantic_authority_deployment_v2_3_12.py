...
def test_rfc3161_verifier_is_executed_and_fail_closed(monkeypatch,tmp_path):
    ...
    monkeypatch.setattr(m,"verify_installed_dependency",lambda *a:None);monkeypatch.setattr(m,"verify_executable",lambda *a:None);monkeypatch.setattr(m.subprocess,"run",lambda *a,**k:next(replies))
