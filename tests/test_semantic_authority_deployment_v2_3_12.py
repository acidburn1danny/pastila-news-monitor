...
def manifest():
    v={"schema":m.SCHEMA,"repository_slug":m.REPOSITORY_SLUG,"repository_id":m.REPOSITORY_ID,"core_runtime_commit":m.RUNTIME_COMMIT,"deployment_runtime_commit":"a"*40,"workflow_commit":"b"*40,"scheduled_utc":"2026-10-01T00:00:00Z","schedule_cron":"0 0 1 10 *","ca_sha256":"1"*64,"cosign_sha256":"2"*64,"launcher_sha256":"3"*64,"trusted_root_sha256":"4"*64,"derivation_policy_identity":"5"*64,"seed_plan_identity":"6"*64,"schedule_precommit_receipt_sha256":"7"*64,"schedule_precommit_verifier_sha256":"8"*64,"schedule_precommit_tsa_root_sha256":"9"*64}
...
def test_rfc3161_verifier_is_executed_and_fail_closed(monkeypatch,tmp_path):
    ...
    v["schedule_precommit_receipt_sha256"]=m.sha(b"receipt");v["schedule_precommit_verifier_sha256"]=m.sha(b"tool");v["schedule_precommit_tsa_root_sha256"]=m.sha(b"root");v["schedule_precommit_payload_sha256"]=m.sha(m.schedule_payload(v));v["deployment_identity"]=m.sha(canonical({k:x for k,x in v.items() if k not in {"deployment_identity","manifest_identity"}}));v["manifest_identity"]=m.sha(canonical({k:x for k,x in v.items() if k!="manifest_identity"}));payload=tmp_path/"payload";payload.write_bytes(m.schedule_payload(v))
