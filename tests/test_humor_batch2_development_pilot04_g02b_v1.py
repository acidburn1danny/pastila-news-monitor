"""Verify Pilot 04's pathless, source-bound, zero-construction G02B release."""
...
def test_g02b_release_is_source_bound_label_blind_and_unconsumed() -> None:
    packet = json.loads((ART / "humor-mechanics-batch2-development-pilot04-constructor-facing-assignment-g02b-v1.json").read_text(encoding="utf-8"))
    release_path = ART / "humor-mechanics-batch2-development-pilot04-constructor-access-release-v1.json"
    release = json.loads(release_path.read_text(encoding="utf-8"))
    audit = json.loads((ART / "humor-mechanics-batch2-development-pilot04-g02b-preconstruction-audit-v1.json").read_text(encoding="utf-8"))
    source = (ART / "humor-mechanics-batch2-development-pilot04-ingestion-v1/source.utf8.txt").read_bytes()
    packet_core = dict(packet); packet_id = packet_core.pop("constructor_facing_packet_identity")
    assert seal("B2_DEVELOPMENT_PILOT04_CONSTRUCTOR_PACKET_G02B_SOURCE_BOUND_V1", packet_core) == packet_id
    assert seal("B2_DEVELOPMENT_PILOT04_CONSTRUCTOR_ACCESS_RELEASE_V1", release["release_core"]) == release["release_identity"]
    audit_core = dict(audit); audit_id = audit_core.pop("audit_identity")
    assert seal("B2_DEVELOPMENT_PILOT04_G02B_AUDIT_V1", audit_core) == audit_id
    source_object = packet["source_object"]
    assert source_object["source_text_utf8"].encode() == source
    assert len(source) == source_object["byte_length"] == 603
