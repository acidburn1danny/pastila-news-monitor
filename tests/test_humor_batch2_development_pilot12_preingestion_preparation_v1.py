...
def test_pilot12_prospective_packet_is_exact_unsigned_and_non_authorizing():
    prospective = json.loads((ART / "humor-mechanics-batch2-development-pilot12-preingestion-v1.json").read_text(encoding="utf-8"))
    independence = json.loads((ART / "humor-mechanics-batch2-development-pilot12-family-independence-v1.json").read_text(encoding="utf-8"))
    packet = json.loads((ART / "humor-mechanics-batch2-development-pilot12-signing-packet-v1.json").read_text(encoding="utf-8"))
    core = dict(prospective); identity = core.pop("preingestion_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT12_PREINGESTION_V1", core)
    core = dict(independence); identity = core.pop("family_independence_identity")
    assert identity == seal("B2_DEVELOPMENT_PILOT12_FAMILY_INDEPENDENCE_V1", core)
    source = (ROOT / "owner-source-pilot12-v1.txt").read_text(encoding="utf-8"); source_bytes = source.encode()
    propositions = prospective["factual_authority_envelope"]["propositions"]
    assert len(propositions) == 8
    for item in propositions:
        span = item["supporting_span"]; cs, ce = span["character_coordinates"]; bs, be = span["utf8_byte_coordinates"]
    ...
    assert prospective["morphological_alignment_opportunity"] == "UNASSIGNED"
