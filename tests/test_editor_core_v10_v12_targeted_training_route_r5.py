...
WORKER = ROOT / "scripts/train_editor_core_v10_v12_targeted_continuation_r5.py"
ROUTE = ROOT / "scripts/run_editor_core_v10_v12_targeted_continuation_r5.sh"
SMOKE = ROOT / "scripts/smoke_editor_core_v10_v12_targeted_training_route_r5.py"
CONFIG = ROOT / "docs/artifacts/editor-core-v10-v12-targeted-continuation-r5-training-config.json"
...
def test_fixture_smoke_closes_without_ml_execution():
    result = load(SMOKE, "r5_route_smoke").run()
...
def test_worker_plan_is_exactly_36_rows_6_steps_and_final_checkpoint():
    worker = load(WORKER, "targeted_r5_worker")
    result = worker.validate_config(json.loads(CONFIG.read_bytes()), worker.EXPECTED_CORPUS, 36)
    assert result["rows"] == 36 and result["optimizer_steps"] == 6
...
def test_worker_rejects_material_contract_drift(mutation):
    worker = load(WORKER, "targeted_r5_worker_negative")
    config = json.loads(CONFIG.read_bytes())
    if mutation == "rows":
        with pytest.raises(ValueError, match="corpus closure"):
            worker.validate_config(config, worker.EXPECTED_CORPUS, 35)
        return
    ...
    with pytest.raises(ValueError, match="identity mismatch|configuration mismatch"):
        worker.validate_config(config, worker.EXPECTED_CORPUS, 36)
...
def test_ml_imports_and_optimizer_are_execution_only():
    ...
    assert "EXPECTED_ROWS = 36" in source and "EXPECTED_STEPS = 6" in source


def test_bound_route_is_isolated_and_binds_r5_sources():
    source = ROUTE.read_text("utf-8")
    assert f"EXPECTED_WORKER={hashlib.sha256(WORKER.read_bytes()).hexdigest()}" in source
    zero_step = ROOT / "scripts/launch_editor_core_v10_v12_targeted_r5_zero_step.py"
    ...
    assert "EDITOR_CORE_R5_TRAINING_OWNER_AUTHORIZED" in source
    assert "EXPECTED_SOURCE_COMMIT=e6ce011f8ba0b4c4d746b7dc0ee330845534d9e7" in source
    assert "EXPECTED_SOURCE_TREE=92412fae59efb64dd798ead55b79fb008c27ed30" in source
    ...
    assert 'python3 -B "$ZERO_STEP" --model "$MODEL" --parent-checkpoint "$PARENT_CHECKPOINT"' in source
    assert 'merge-base --is-ancestor "$EXPECTED_SOURCE_COMMIT" origin/successor/core-v2-v12-runner-binding-remediation' in source
