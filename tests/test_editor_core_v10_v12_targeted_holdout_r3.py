...
RUNNER = ROOT / "scripts/evaluate_editor_core_v10_v12_targeted_holdout_r3.py"
AUDITOR = ROOT / "scripts/audit_editor_core_v10_v12_targeted_holdout_r3.py"
SHELL = ROOT / "scripts/run_editor_core_v10_v12_targeted_holdout_r3.sh"
...
def test_requests_are_complete_and_target_free():
    runner = load(RUNNER, "r3_runner")
    rows = runner.validate_requests(ROOT / "docs/artifacts/editor-core-v10-v12-targeted-continuation-r3-holdout-requests.jsonl")
    assert len({runner.request_identity(row) for row in rows}) == 12


def test_fixture_smoke_and_leakage_rejection():
    runner = load(RUNNER, "r3_smoke")
...
def test_auditor_closes_receipt_rows_and_classes(tmp_path):
    auditor = load(AUDITOR, "r3_auditor")
...
def test_semantic_audit_detects_epistemic_regression():
    auditor = load(AUDITOR, "r3_semantic")
...
def test_auditor_rejects_extra_output(tmp_path):
    ...
    with pytest.raises(ValueError, match="closure"):
        load(AUDITOR, "r3_extra").audit(output, tmp_path / "key", "x", "a", "q", "r")
...
def test_shell_binds_only_the_two_expected_adapters_and_excludes_answer_key():
    ...
    assert "r3-step-6" in source and "eaa142cb58c2b795e75ce1b57746950e6fcd3026bc5dd2ac8d55c210c5ce6a00" in source
    assert "EXPECTED_PARENT_COMMIT=ba5220431a28ac8a83e0671a07be83ba990cda49" in source
