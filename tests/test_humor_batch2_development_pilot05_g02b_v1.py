"""Verify Pilot 05's pathless, source-bound, zero-construction G02B release."""
...
def test_g02b_release_is_source_bound_label_blind_and_unconsumed() -> None:
    packet = json.loads((ART / "humor-mechanics-batch2-development-pilot05-constructor-facing-assignment-g02b-v1.json").read_text(encoding="utf-8"))
    release_path = ART / "humor-mechanics-batch2-development-pilot05-constructor-access-release-v1.json"
    release = json.loads(release_path.read_text(encoding="utf-8"))
    audit = json.loads((ART / "humor-mechanics-batch2-development-pilot05-g02b-preconstruction-audit-v1.json").read_text(encoding="utf-8"))
    source = (ART / "humor-mechanics-batch2-development-pilot05-ingestion-v1/source.utf8.txt").read_bytes()
    packet_core = dict(packet); packet_id = packet_core.pop("constructor_facing_packet_identity")
    assert seal("B2_DEVELOPMENT_PILOT05_CONSTRUCTOR_PACKET_G02B_SOURCE_BOUND_V1", packet_core) == packet_id
    assert seal("B2_DEVELOPMENT_PILOT05_CONSTRUCTOR_ACCESS_RELEASE_V1", release["release_core"]) == release["release_identity"]
    audit_core = dict(audit); audit_id = audit_core.pop("audit_identity")
    assert seal("B2_DEVELOPMENT_PILOT05_G02B_AUDIT_V1", audit_core) == audit_id
    source_object = packet["source_object"]
    assert source_object["source_text_utf8"].encode() == source
    assert len(source) == source_object["byte_length"] == 550
    assert hashlib.sha256(source).hexdigest() == source_object["sha256"]
    visible = canonical(packet).lower()
    for token in (b"m13", b"absurd_logical_extension", b"mapping_commitment", b"conformance_schema", b"removal_test", b"misdirection", b"escalation", b"hyperbole", b"rebalanc", b"g04b", b"pool"):
