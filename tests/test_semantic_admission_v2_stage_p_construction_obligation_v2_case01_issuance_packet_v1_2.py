...
from scripts.verify_construction_obligation_v2_case01_issuance_packet_v1_2 import verify
...
def test_single_command_and_fail_closed_limits_are_exact() -> None:
    ...
    assert any(item.endswith("linux_generation_runner_v1_2") for item in command)
...
def test_materializer_and_verifier_have_no_execution_callsite() -> None:
    paths = (
        ...
        "stage_p_construction_obligation_v2_case01_issuance_packet_v1_2.py",
        ROOT / "scripts/verify_construction_obligation_v2_case01_issuance_packet_v1_2.py",
    )
    for path in paths:
