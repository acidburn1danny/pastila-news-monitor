"""Verify all Pilot 02 custodial proofs and atomically materialize ingestion."""
...
FREEZE_COMMIT = "8b60e13145386ca4cbf379e1ee57a799c0c1de5f"
PRE_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot02-preingestion-v1.json"
PACKET_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot02-signing-packet-v1.json"
...
PRIOR_LEDGER_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot01-ingestion-v1/access-ledger-segment.json"
SOURCE = ROOT / "owner-source-pilot02-v1.txt"
DECLARATION = ROOT / "owner-declaration-pilot02-v1.json"
DESTINATION = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot02-ingestion-v1"
...
def verify_signature(public_pem: str, algorithm: str, message: bytes, signature: bytes) -> bool:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot02-verify-") as temp:
        folder = Path(temp)
...
def spki_fingerprint(public_pem: str) -> str:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot02-spki-") as temp:
...
def verify(responses: Path) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    if packet["packet_identity"] != "faeba3b95de0548ed55bdb0aca0b037b7bdb38166cb2f8e7ac8d0a749c86a63f":
        raise SystemExit("packet identity")
    if packet["packet_identity"] != seal("B2_DEVELOPMENT_PILOT02_SIGNING_PACKET_V1", packet["packet_core"]):
    ...
    paths = sorted(responses.glob("*.pilot02-response.json"))
    ...
    for path in paths:
        ...
        if seal("B2_PILOT02_SIGNING_CHALLENGE_V1", core) != identity:
        ...
        if not (response["packet_identity"] == packet["packet_identity"] and
                ...
                challenge["domain"] == "PASTILA_BATCH2_DEVELOPMENT_PILOT02_PREINGESTION_V1" and
    ...
    allowed = {f"HEAD:{PACKET_PATH}", f"HEAD:{PRE_PATH}",
               "HEAD:docs/humor-mechanics-batch2-development-pilot02-owner-signing-v1.md"}
...
def materialize(pre: dict[str, Any], packet: dict[str, Any], verified: list[dict[str, Any]], prior_ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    source, declaration = SOURCE.read_bytes(), DECLARATION.read_bytes()
    response_set_identity = seal("B2_DEVELOPMENT_PILOT02_VERIFIED_RESPONSE_SET_V1", verified)
    previous = prior_ledger["final_ledger_head"]
    entries = []
    for sequence, operation in enumerate(packet["packet_core"]["operations"], start=22):
        ...
        entry = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                 ...
                 "event": "DEVELOPMENT_PILOT02_ATOMIC_INGESTION_OPERATION_ADMITTED",
    ...
    completion = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                  "entry_sequence": 28, "previous_entry_hash": previous,
                  "event": "DEVELOPMENT_PILOT02_ATOMIC_INGESTION_COMPLETED",
    ...
    ledger = {"schema_name": "batch2-development-pilot02-access-ledger-segment-v1", "schema_version": "1.0.0",
              "previous_ledger_head": prior_ledger["final_ledger_head"], "entries": entries,
              "final_ledger_head": completion["entry_hash"]}
    ledger["ledger_segment_identity"] = seal("B2_DEVELOPMENT_PILOT02_ACCESS_LEDGER_SEGMENT_V1", ledger)
    ...
    archive_receipt = {"schema_name": "batch2-development-pilot02-immutable-archive-receipt-v1", "schema_version": "1.0.0",
                       "immutable_archive_commitment": pre["immutable_archive_commitment"],
    ...
    archive_receipt["archive_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT02_ARCHIVE_RECEIPT_V1", archive_receipt)
    evidence = {"schema_name": "batch2-development-pilot02-custodial-verification-v1", "schema_version": "1.0.0",
    ...
    evidence["verification_identity"] = seal("B2_DEVELOPMENT_PILOT02_CUSTODIAL_VERIFICATION_V1", evidence)
    receipt = {"schema_name": "batch2-development-pilot02-ingestion-receipt-v1", "schema_version": "1.0.0",
    ...
    receipt["ingestion_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT02_INGESTION_RECEIPT_V1", receipt)
    parent = DESTINATION.parent
    temporary = Path(tempfile.mkdtemp(prefix=".pilot02-ingestion-", dir=parent))
    try:
