"""Verify all Pilot 09 custodial proofs and atomically materialize ingestion."""
...
FREEZE_COMMIT = "c418c0974ec86d7bfc2d5880969531d7b09325d5"
HELPER_COMMIT = "da51de62b43cc2f7346d554e05fbf763a9a3cd5e"
PRE_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot09-preingestion-v1.json"
PACKET_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot09-signing-packet-v1.json"
REG_PATH = "docs/artifacts/humor-mechanics-batch2-custodial-public-key-registration-v1.json"
PRIOR_LEDGER_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot08-ingestion-v1/access-ledger-segment.json"
SOURCE = ROOT / "owner-source-pilot09-v1.txt"
DECLARATION = ROOT / "owner-declaration-pilot09-v1.json"
DESTINATION = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot09-ingestion-v1"
SOURCE_SHA = "608f26b4588c347707ae5eccb08194d498fb3b3e9e7a6402be63ad2bc7c77c77"
DECLARATION_SHA = "8c68d5bf2a711fc518879fcddfba9ea44d7c232fb962fdecc816bf97d249b41b"
PACKET_ID = "b502489eff80bf991d93335421dad36bf173cef4c55a031f268b8c236c37ca08"
HANDOFF = Path.home() / "Pastila-Owner-Handoff" / "Batch2-Development-Pilot09-v1"
...
def verify_signature(public_pem: str, algorithm: str, message: bytes, signature: bytes) -> bool:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot09-verify-") as temp:
        folder = Path(temp)
...
def spki_fingerprint(public_pem: str) -> str:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot09-spki-") as temp:
...
def verify(responses: Path) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    if packet["packet_identity"] != PACKET_ID or packet["packet_identity"] != seal("B2_DEVELOPMENT_PILOT09_SIGNING_PACKET_V1", packet["packet_core"]):
    ...
    paths = sorted(responses.glob("*.pilot09-response.json"))
    ...
    for path in paths:
        ...
        if seal("B2_PILOT09_SIGNING_CHALLENGE_V1", core) != identity:
        ...
        if not (response["packet_identity"] == packet["packet_identity"] and response["purpose"] == request["purpose"] == challenge["purpose"]
                ...
                and challenge["domain"] == "PASTILA_BATCH2_DEVELOPMENT_PILOT09_PREINGESTION_V1"
...
def materialize(pre: dict[str, Any], packet: dict[str, Any], verified: list[dict[str, Any]], prior: dict[str, Any]) -> dict[str, Any]:
    ...
    source, declaration = SOURCE.read_bytes(), DECLARATION.read_bytes()
    response_set = seal("B2_DEVELOPMENT_PILOT09_VERIFIED_RESPONSE_SET_V1", verified)
    previous, entries = prior["final_ledger_head"], []
    for sequence, operation in enumerate(packet["packet_core"]["operations"], start=71):
        entry = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                 ...
                 "event": "DEVELOPMENT_PILOT09_ATOMIC_INGESTION_OPERATION_ADMITTED", "packet_identity": packet["packet_identity"],
    ...
    completion = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                  "entry_sequence": 77, "previous_entry_hash": previous, "event": "DEVELOPMENT_PILOT09_ATOMIC_INGESTION_COMPLETED",
    ...
    ledger = {"schema_name": "batch2-development-pilot09-access-ledger-segment-v1", "schema_version": "1.0.0",
              "previous_ledger_head": prior["final_ledger_head"], "entries": entries, "final_ledger_head": completion["entry_hash"]}
    ledger["ledger_segment_identity"] = seal("B2_DEVELOPMENT_PILOT09_ACCESS_LEDGER_SEGMENT_V1", ledger)
    ...
    archive = {"schema_name": "batch2-development-pilot09-immutable-archive-receipt-v1", "schema_version": "1.0.0",
    ...
    archive["archive_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT09_ARCHIVE_RECEIPT_V1", archive)
    evidence = {"schema_name": "batch2-development-pilot09-custodial-verification-v1", "schema_version": "1.0.0",
    ...
    evidence["verification_identity"] = seal("B2_DEVELOPMENT_PILOT09_CUSTODIAL_VERIFICATION_V1", evidence)
    receipt = {"schema_name": "batch2-development-pilot09-ingestion-receipt-v1", "schema_version": "1.0.0",
               ...
               "partition_identity": pre["prospective_partition_identity"], "creative_premise_family_id": "UNASSIGNED",
               "proposition_sufficiency_evaluated": False, "constructor_v5_compatibility_evaluated": False, "verification_identity": evidence["verification_identity"],
               ...
               "authority_matrix": {key: False for key in ("g01_admission", "proposition_sufficiency_evaluation", "assignment",
                                                            "constructor_source_compatibility_evaluation", "constructor_release", "construction", "g04b_pool_certification", "model_exposure",
                                                            "training", "runtime_integration", "production_routing")}}
    receipt["ingestion_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT09_INGESTION_RECEIPT_V1", receipt)
    temporary = Path(tempfile.mkdtemp(prefix=".pilot09-ingestion-", dir=DESTINATION.parent))
    try:
