"""Offline worker for one bound Bridge A1/A2 arm and seed."""
...
CHECKPOINT_STEPS = (12,)
EXPECTED_CORPUS = {
    "A1": "f3461bfcc83d199c4517db08fafaa05dfab31e4a6d3ec98519c33ca9ff34df48",
    "A2": "10b7fda096e2155f1bbc199268ba60fb37432e6317806cfe51d305d45cb3afc5",
}
EXPECTED_PROTOCOL = "6eed58beaadf277d30e60282260bc9ef513d90fa5b49a3bac03f64ad56a2e0bb"
...
def validate_config(config: dict[str, object], corpus_sha256: str, row_count: int) -> dict[str, object]:
    identity = config.get("run_spec_identity")
    core = {key: value for key, value in config.items() if key != "run_spec_identity"}
    if identity != hashlib.sha256(canonical(core)).hexdigest():
        raise ValueError("run spec identity mismatch")
    arm, seed = config.get("arm"), config.get("seed")
    if arm not in EXPECTED_CORPUS or type(seed) is not int or seed not in SEEDS:
        raise ValueError("run arm or seed")
    expected = {
        "schema": "editor-core-bridge-causal-pilot-run-spec", "schema_version": 1,
        "arm": arm, "seed": seed, "output_slug": f"{arm.lower()}-seed-{seed}",
        "published_bridge_commit": "94880823ac060539f21b1df4b8374b04a484ad78",
        "successor_protocol_sha256": EXPECTED_PROTOCOL,
        "parent_adapter_identity": EXPECTED_PARENT,
        "parent_checkpoint_identity": EXPECTED_CHECKPOINT,
        "training_corpus_sha256": EXPECTED_CORPUS[arm],
        "training_rows": 96, "new_rows": 48, "replay_rows": 48,
        ...
        "holdout_training_or_inference_use": False,
        "r3_r6_parent_use": False,
        "training_authorized": False,
    }
    if config != {**expected, "run_spec_identity": identity}:
        raise ValueError("frozen run specification mismatch")
    if corpus_sha256 != EXPECTED_CORPUS[arm] or row_count != EXPECTED_ROWS:
...
def run_training(model_path: Path, parent_path: Path, corpus_path: Path, config_path: Path, output: Path) -> dict[str, object]:
    required = {"TRAINING_EXECUTION_AUTHORIZED", "MODEL_SHA256", "PARENT_SHA256", "CORPUS_SHA256", "CONFIG_SHA256", "ROOTFS_SHA256", "LAUNCHER_SHA256", "WORKER_SHA256", "RUN_SPEC_IDENTITY", "OUTPUT_SLUG"}
    ...
    if config["run_spec_identity"] != os.environ["RUN_SPEC_IDENTITY"] or \
            config["output_slug"] != os.environ["OUTPUT_SLUG"]:
        raise ValueError("run/output substitution")
    ...
    authority = {"arm": config["arm"], "seed": seed, "output_slug": config["output_slug"],
                 "run_spec_identity": config["run_spec_identity"], "successor_protocol_sha256": EXPECTED_PROTOCOL,
                 "corpus_sha256": corpus_sha, "config_sha256": config_sha, "model_sha256": model_identity,
                 "parent_sha256": parent_identity, "parent_checkpoint_identity": EXPECTED_CHECKPOINT,
                 "rootfs_sha256": os.environ["ROOTFS_SHA256"], "launcher_sha256": os.environ["LAUNCHER_SHA256"],
    ...
    receipt = {"schema": "editor-core-bridge-causal-pilot-training-receipt", "schema_version": 1,
               **authority, "optimizer": "PAGED_ADAMW_8BIT_NEW_STATE", "optimizer_steps": steps,
               "checkpoint_identities": checkpoints, "final_loss": format(losses[-1], ".17g"),
               "network_activity": False, "development_only": True, "holdout_used": False,
               "r3_r6_parent_use": False, "adjudication_performed": False, "promotion": False}
...
def fixture_smoke(config: dict[str, object]) -> dict[str, object]:
    plan = validate_config(config, EXPECTED_CORPUS[config["arm"]], EXPECTED_ROWS)
    core = {"schema": "editor-core-bridge-causal-pilot-worker-fixture-smoke", "schema_version": 1,
            "status": "PASS_FIXTURE_ONLY", "arm": config["arm"], "seed": config["seed"], **plan,
            "model_loaded": False, "optimizer_created": False, "optimizer_steps_executed": 0,
