"""Verify all Pilot 07 custodial proofs and atomically materialize ingestion."""
...
FREEZE_COMMIT = "5b61ac473335352348149051d3c06ea7b553af8f"
HELPER_COMMIT = "efb95e4a4106c59cdcd2c655708ede5a27822836"
PRE_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot07-preingestion-v1.json"
PACKET_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot07-signing-packet-v1.json"
REG_PATH = "docs/artifacts/humor-mechanics-batch2-custodial-public-key-registration-v1.json"
PRIOR_LEDGER_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot06-ingestion-v1/access-ledger-segment.json"
SOURCE = ROOT / "owner-source-pilot07-v1.txt"
DECLARATION = ROOT / "owner-declaration-pilot07-v1.json"
DESTINATION = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot07-ingestion-v1"
...
HANDOFF = Path.home() / "Pastila-Owner-Handoff" / "Batch2-Development-Pilot07-v1"
...
def verify_signature(public_pem: str, algorithm: str, message: bytes, signature: bytes) -> bool:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot07-verify-") as temp:
        folder = Path(temp)
...
def spki_fingerprint(public_pem: str) -> str:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot07-spki-") as temp:
...
def verify(responses: Path) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    if packet["packet_identity"] != PACKET_ID or packet["packet_identity"] != seal("B2_DEVELOPMENT_PILOT07_SIGNING_PACKET_V1", packet["packet_core"]):
    ...
    paths = sorted(responses.glob("*.pilot07-response.json"))
    ...
    for path in paths:
        ...
        if seal("B2_PILOT07_SIGNING_CHALLENGE_V1", core) != identity:
        ...
        if not (response["packet_identity"] == packet["packet_identity"] and response["purpose"] == request["purpose"] == challenge["purpose"]
                ...
                and challenge["domain"] == "PASTILA_BATCH2_DEVELOPMENT_PILOT07_PREINGESTION_V1"
...
def materialize(pre: dict[str, Any], packet: dict[str, Any], verified: list[dict[str, Any]], prior: dict[str, Any]) -> dict[str, Any]:
    ...
    source, declaration = SOURCE.read_bytes(), DECLARATION.read_bytes()
    response_set = seal("B2_DEVELOPMENT_PILOT07_VERIFIED_RESPONSE_SET_V1", verified)
    previous, entries = prior["final_ledger_head"], []
    for sequence, operation in enumerate(packet["packet_core"]["operations"], start=57):
        entry = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                 ...
                 "event": "DEVELOPMENT_PILOT07_ATOMIC_INGESTION_OPERATION_ADMITTED", "packet_identity": packet["packet_identity"],
    ...
    completion = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                  "entry_sequence": 63, "previous_entry_hash": previous, "event": "DEVELOPMENT_PILOT07_ATOMIC_INGESTION_COMPLETED",
    ...
    ledger = {"schema_name": "batch2-development-pilot07-access-ledger-segment-v1", "schema_version": "1.0.0",
              "previous_ledger_head": prior["final_ledger_head"], "entries": entries, "final_ledger_head": completion["entry_hash"]}
    ledger["ledger_segment_identity"] = seal("B2_DEVELOPMENT_PILOT07_ACCESS_LEDGER_SEGMENT_V1", ledger)
    ...
    archive = {"schema_name": "batch2-development-pilot07-immutable-archive-receipt-v1", "schema_version": "1.0.0",
    ...
    archive["archive_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT07_ARCHIVE_RECEIPT_V1", archive)
    evidence = {"schema_name": "batch2-development-pilot07-custodial-verification-v1", "schema_version": "1.0.0",
    ...
    evidence["verification_identity"] = seal("B2_DEVELOPMENT_PILOT07_CUSTODIAL_VERIFICATION_V1", evidence)
    receipt = {"schema_name": "batch2-development-pilot07-ingestion-receipt-v1", "schema_version": "1.0.0",
    ...
    receipt["ingestion_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT07_INGESTION_RECEIPT_V1", receipt)
    temporary = Path(tempfile.mkdtemp(prefix=".pilot07-ingestion-", dir=DESTINATION.parent))
    try:
