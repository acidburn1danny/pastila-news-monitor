"""Verify all Pilot 06 custodial proofs and atomically materialize ingestion."""
...
FREEZE_COMMIT = "3dfd06df40ebb20040efb1aeb0eccecfcd0042e0"
HELPER_COMMIT = "bddcd9dc35feaca775c1d8e8c3b7984ef6a93d9a"
PRE_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot06-preingestion-v1.json"
PACKET_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot06-signing-packet-v1.json"
REG_PATH = "docs/artifacts/humor-mechanics-batch2-custodial-public-key-registration-v1.json"
PRIOR_LEDGER_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot05-ingestion-v1/access-ledger-segment.json"
SOURCE = ROOT / "owner-source-pilot06-v1.txt"
DECLARATION = ROOT / "owner-declaration-pilot06-v1.json"
DESTINATION = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot06-ingestion-v1"
...
HANDOFF = Path.home() / "Pastila-Owner-Handoff" / "Batch2-Development-Pilot06-v1"
...
def verify_signature(public_pem: str, algorithm: str, message: bytes, signature: bytes) -> bool:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot06-verify-") as temp:
        folder = Path(temp)
...
def spki_fingerprint(public_pem: str) -> str:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot06-spki-") as temp:
...
def verify(responses: Path) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    if packet["packet_identity"] != PACKET_ID or packet["packet_identity"] != seal("B2_DEVELOPMENT_PILOT06_SIGNING_PACKET_V1", packet["packet_core"]):
    ...
    paths = sorted(responses.glob("*.pilot06-response.json"))
    ...
    for path in paths:
        ...
        if seal("B2_PILOT06_SIGNING_CHALLENGE_V1", core) != identity:
        ...
        if not (response["packet_identity"] == packet["packet_identity"] and response["purpose"] == request["purpose"] == challenge["purpose"]
                ...
                and challenge["domain"] == "PASTILA_BATCH2_DEVELOPMENT_PILOT06_PREINGESTION_V1"
...
def materialize(pre: dict[str, Any], packet: dict[str, Any], verified: list[dict[str, Any]], prior: dict[str, Any]) -> dict[str, Any]:
    ...
    source, declaration = SOURCE.read_bytes(), DECLARATION.read_bytes()
    response_set = seal("B2_DEVELOPMENT_PILOT06_VERIFIED_RESPONSE_SET_V1", verified)
    previous, entries = prior["final_ledger_head"], []
    for sequence, operation in enumerate(packet["packet_core"]["operations"], start=50):
        entry = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                 ...
                 "event": "DEVELOPMENT_PILOT06_ATOMIC_INGESTION_OPERATION_ADMITTED", "packet_identity": packet["packet_identity"],
    ...
    completion = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                  "entry_sequence": 56, "previous_entry_hash": previous, "event": "DEVELOPMENT_PILOT06_ATOMIC_INGESTION_COMPLETED",
    ...
    ledger = {"schema_name": "batch2-development-pilot06-access-ledger-segment-v1", "schema_version": "1.0.0",
              "previous_ledger_head": prior["final_ledger_head"], "entries": entries, "final_ledger_head": completion["entry_hash"]}
    ledger["ledger_segment_identity"] = seal("B2_DEVELOPMENT_PILOT06_ACCESS_LEDGER_SEGMENT_V1", ledger)
    ...
    archive = {"schema_name": "batch2-development-pilot06-immutable-archive-receipt-v1", "schema_version": "1.0.0",
    ...
    archive["archive_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT06_ARCHIVE_RECEIPT_V1", archive)
    evidence = {"schema_name": "batch2-development-pilot06-custodial-verification-v1", "schema_version": "1.0.0",
    ...
    evidence["verification_identity"] = seal("B2_DEVELOPMENT_PILOT06_CUSTODIAL_VERIFICATION_V1", evidence)
    receipt = {"schema_name": "batch2-development-pilot06-ingestion-receipt-v1", "schema_version": "1.0.0",
    ...
    receipt["ingestion_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT06_INGESTION_RECEIPT_V1", receipt)
    temporary = Path(tempfile.mkdtemp(prefix=".pilot06-ingestion-", dir=DESTINATION.parent))
    try:
