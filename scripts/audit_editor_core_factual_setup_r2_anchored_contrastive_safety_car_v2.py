...
def main():
 ...
 assert "corrective_nonempty_token_mappings" in pre and "replay_empty_span_rows" in pre and "zero-step.json" in pre
 assert sup.index("auth=verify") < sup.index("slots=plan") and "program-failure.json" in sup
 assert "replay critical spans forbidden" in worker and "corrective critical spans required" in worker and "eligible_evidence\":False" in worker
 for path in (pre,sup):
