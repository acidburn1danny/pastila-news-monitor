"""Fixture-only smoke for the R4 targeted training worker and bound route."""
...
WORKER = ROOT / "scripts/train_editor_core_v10_v12_targeted_continuation_r4.py"
ROUTE = ROOT / "scripts/run_editor_core_v10_v12_targeted_continuation_r4.sh"
ZERO_STEP = ROOT / "scripts/launch_editor_core_v10_v12_targeted_r4_zero_step.py"
FIXTURE = ROOT / "tests/fixtures/editor_core_targeted_training_route_r4/training-config.json"


def run() -> dict[str, object]:
    spec = importlib.util.spec_from_file_location("targeted_r4_worker", WORKER)
    ...
    required = ["unshare --kill-child=KILL --mount --net --pid --ipc --uts --fork", "TRAINING_EXECUTION_AUTHORIZED=1", "mount -o remount,bind,ro", "EXPECTED_SOURCE_COMMIT=4d55b41c591454305b95b2f32b4259d77b6f3db1", "EXPECTED_SOURCE_TREE=d950f57fe89dc39e336176f9ae105186ead757fd", 'GIT=(git -c "safe.directory=$REPOSITORY" -C "$REPOSITORY")', '"${GIT[@]}" merge-base --is-ancestor', "EXPECTED_ZERO_STEP=", "EXPECTED_WORKER=", "EXPECTED_CORPUS=", "--execute-authorized"]
