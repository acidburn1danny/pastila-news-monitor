"""Read-only closure and comparison audit for R2 holdout outputs."""
...
def audit(output: Path, answer_key: Path, candidate: str, adapter_sha256: str, requests_sha256: str, runner_sha256: str) -> dict[str, object]:
    ...
    core = {
        "schema": "pastila-editor-core-targeted-r2-holdout-audit", "schema_version": 1,
        "candidate": candidate, "rows": EXPECTED_ROWS, "structural_passed": structural,
        "exact_target_passed": exact, "failure_class_counts": dict(sorted(counts.items())),
...
def compare(left: Path, right: Path, answer_key: Path) -> dict[str, object]:
    ...
    core = {
        "schema": "pastila-editor-core-targeted-r2-holdout-comparison", "schema_version": 1,
