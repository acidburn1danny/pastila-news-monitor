"""Verify all Pilot 01 custodial proofs and atomically materialize ingestion."""
...
FREEZE_COMMIT = "20f22085f2344537417ae8529ff6ea269498fec9"
PRE_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot01-preingestion-v1.json"
PACKET_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot01-signing-packet-v1.json"
...
PRIOR_LEDGER_PATH = "docs/artifacts/humor-mechanics-batch2-custodial-activation-preflight-ledger-v1.json"
SOURCE = ROOT / "owner-source-v1.txt"
DECLARATION = ROOT / "owner-declaration-v1.json"
DESTINATION = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot01-ingestion-v1"
...
def verify_signature(public_pem: str, algorithm: str, message: bytes, signature: bytes) -> bool:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot01-verify-") as temp:
        folder = Path(temp)
...
def spki_fingerprint(public_pem: str) -> str:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot01-spki-") as temp:
...
def verify(responses: Path) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    if subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip() != FREEZE_COMMIT:
    ...
    if packet["packet_identity"] != "5e02059125ffe6a8553eb2cbc7ffc1cca7b98201ba905cfe59dad50aa1e6ac75":
        raise SystemExit("packet identity")
    if packet["packet_identity"] != seal("B2_DEVELOPMENT_PILOT01_SIGNING_PACKET_V1", packet["packet_core"]):
    ...
    paths = sorted(responses.glob("*.pilot01-response.json"))
    ...
    for path in paths:
        ...
        if seal("B2_PILOT01_SIGNING_CHALLENGE_V1", core) != identity:
            raise SystemExit(f"challenge seal: {key}")
        if not (response["packet_identity"] == packet["packet_identity"] and
                response["purpose"] == request["purpose"] == challenge["purpose"] and
                response["principal_identity"] == challenge["principal_identity"] == registered["principal_identity"] and
                response["challenge_identity"] == challenge["challenge_identity"] and
                challenge["role"] == response["signer_role"] and
                challenge["domain"] == "PASTILA_BATCH2_DEVELOPMENT_PILOT01_PREINGESTION_V1" and
                challenge["object_identity"] == packet["packet_core"]["operations"][request["operation_ordinal"]]["object_identity"] and
                challenge["prior_ledger_head"] == prior_ledger["final_ledger_head"] and
                challenge["preingestion_identity"] == pre["preingestion_identity"] and
                challenge["source_sha256"] == SOURCE_SHA and challenge["declaration_sha256"] == DECLARATION_SHA and
                challenge["grants_operational_content_access"] is False):
    ...
    for op in packet["packet_core"]["operations"]:
        if by_op.get(op["ordinal"]) != set(op["required_signer_roles"]):
            raise SystemExit(f"countersignature set: {op['ordinal']}")
        if op["distinct_signers_required"] and len(by_op[op["ordinal"]]) != len(op["required_signer_roles"]):
            raise SystemExit(f"separation of duties: {op['ordinal']}")
    if len({keys[x["role"]]["public_key_fingerprint"] for x in verified}) != 5:
        raise SystemExit("role-key separation mismatch")
    # No prior committed artifact may have consumed this packet or nonce set.
    ...
    allowed = {f"HEAD:{PACKET_PATH}", f"HEAD:{PRE_PATH}",
               "HEAD:docs/humor-mechanics-batch2-development-pilot01-owner-signing-v1.md"}
...
def materialize(pre: dict[str, Any], packet: dict[str, Any], verified: list[dict[str, Any]], prior_ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    source, declaration = SOURCE.read_bytes(), DECLARATION.read_bytes()
    response_set_identity = seal("B2_DEVELOPMENT_PILOT01_VERIFIED_RESPONSE_SET_V1", verified)
    previous = prior_ledger["final_ledger_head"]
    entries = []
    for sequence, operation in enumerate(packet["packet_core"]["operations"], start=15):
        ...
        entry = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                 ...
                 "event": "DEVELOPMENT_PILOT01_ATOMIC_INGESTION_OPERATION_ADMITTED",
    ...
    completion = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                  "entry_sequence": 21, "previous_entry_hash": previous,
                  "event": "DEVELOPMENT_PILOT01_ATOMIC_INGESTION_COMPLETED",
                  "packet_identity": packet["packet_identity"], "source_package_identity": pre["source_package_identity"],
                  "partition_identity": pre["partition_identity"], "response_set_identity": response_set_identity,
    ...
    ledger = {"schema_name": "batch2-development-pilot01-access-ledger-segment-v1", "schema_version": "1.0.0",
              "previous_ledger_head": prior_ledger["final_ledger_head"], "entries": entries,
              "final_ledger_head": completion["entry_hash"]}
    ledger["ledger_segment_identity"] = seal("B2_DEVELOPMENT_PILOT01_ACCESS_LEDGER_SEGMENT_V1", ledger)
    source_package = {"schema_name": "batch2-owned-authority-source-package-materialization-v1", "schema_version": "1.0.0",
                      ...
                      "partition_identity": pre["partition_identity"], "creative_premise_family_id": "UNASSIGNED",
                      "access_ledger_identity": ledger["final_ledger_head"], "materialization_status": "INGESTED_IMMUTABLE"}
    archive_receipt = {"schema_name": "batch2-development-pilot01-immutable-archive-receipt-v1", "schema_version": "1.0.0",
                       "immutable_archive_commitment": pre["immutable_archive_commitment"],
    ...
    archive_receipt["archive_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT01_ARCHIVE_RECEIPT_V1", archive_receipt)
    evidence = {"schema_name": "batch2-development-pilot01-custodial-verification-v1", "schema_version": "1.0.0",
    ...
    evidence["verification_identity"] = seal("B2_DEVELOPMENT_PILOT01_CUSTODIAL_VERIFICATION_V1", evidence)
    receipt = {"schema_name": "batch2-development-pilot01-ingestion-receipt-v1", "schema_version": "1.0.0",
               ...
               "authority_matrix": {"mechanism_assignment": False, "creative_premise_assignment": False, "humor_construction": False,
                                    "candidate_generation": False, "model_exposure": False, "training": False,
                                    "runtime_integration": False, "production_routing": False}}
    receipt["ingestion_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT01_INGESTION_RECEIPT_V1", receipt)
    parent = DESTINATION.parent
    temporary = Path(tempfile.mkdtemp(prefix=".pilot01-ingestion-", dir=parent))
    try:
