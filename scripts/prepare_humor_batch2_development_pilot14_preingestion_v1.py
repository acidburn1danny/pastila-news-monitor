"""Prepare Pilot 14 prospective identities and an unsigned signing packet only."""
...
DECLARATION = ROOT / "owner-declaration-pilot14-v1.json"
VALIDATION_COMMIT = "d6b8ae4f3b3eef89ccbb63d0382913d40133201f"
VALIDATION_IDENTITY = "da9ca0601770310dc6bf273940dcec7141e1f18b48eed8df0753712d5dc377b1"
SOURCE_SHA256 = "aec2bccf7ec0cc9f059785d00be1fe891a5b4c64b834da0cb9743518ff9e512d"
DECLARATION_SHA256 = "6f00f326d4f40d82bb856bb1d737f4b859af8f9b0285168d36b91913f01b5580"
LEDGER_HEAD = "a97d53ffe4ce3eedd3438e3551b9ee94b67f2f515e016783359efe290b93a72a"
...
def main() -> None:
    ...
    validation = git_json("docs/artifacts/humor-mechanics-batch2-development-pilot14-strict-preingestion-validation-v1.json")
    ...
    require(validation["deterministic_blockers"] == [] and validation["repair_state"] == "NONE_INPUTS_VALIDATED_BYTE_EXACT", "validation boundary")
    ...
    propositions = [
        {"proposition_id": f"P{i+1}", "supporting_span": {**coords(source, start, end), "span_sha256": sha256(source[start:end].encode())}, "statement": source[start:end], "modality": "ASSERTED", "scope": "OWNER_AUTHORED_SYNTHETIC_TEST_UNIVERSE_ONLY", "selection": "NOT_PERFORMED"}
    ...
    independence_core = {"schema_name": "batch2-development-pilot14-family-independence-v1", "schema_version": "1.0.0", "pilot14_source_sha256": SOURCE_SHA256, "prior_source_sha256": prior_hashes, "prior_family_identities": {f"pilot{i:02d}": item["g01b"]["family_identities"] for i, item in enumerate(admissions, 1)}, "pilot14_topology": ["SYNTHETIC_ACOUSTIC_SENSOR_INSTALLATION", "SERIAL_TO_NUMBERED_POSITION_ASSOCIATION", "PREINSTALLATION_SERIAL_AND_POSITION_VERIFICATION", "INDIVIDUAL_POSITION_INSTALLATION", "ACTUAL_POSITION_AND_TIME_LOGGING", "INSTALLATION_FAILURE_NONMOUNT_AND_SEPARATE_LOG", "CAMPAIGN_ONLY_SCOPE", "POSITION_USABILITY_AND_NONINSTALLATION_UNKNOWN"], "source_hash_distinct": SOURCE_SHA256 not in prior_hashes, "git_blob_distinct": source_blob not in prior_blobs, "exact_prior_line_reuse": False, "source_event_topic_revision_sibling_syndication_same_event_relation": False, "prior_downstream_or_expected_result_shaping": False, "blind_family_access": False, "result": "PASS_FRESH_FAMILY_INDEPENDENCE"}
    independence = {**independence_core, "family_independence_identity": seal("B2_DEVELOPMENT_PILOT14_FAMILY_INDEPENDENCE_V1", independence_core)}
    ...
    packet_identity = seal("B2_DEVELOPMENT_PILOT14_SIGNING_PACKET_V1", packet_core)
    requests = []
    for operation in records:
        for role in operation["required_signer_roles"]:
            challenge_core = {"domain": "PASTILA_BATCH2_DEVELOPMENT_PILOT14_PREINGESTION_V1", "purpose": operation["purpose"], "role": role, "principal_identity": principals[role], "object_identity": operation["object_identity"], "packet_identity": packet_identity, "source_sha256": SOURCE_SHA256, "declaration_sha256": DECLARATION_SHA256, "preingestion_identity": prospective["preingestion_identity"], "nonce": seal("B2_PILOT14_SIGNING_NONCE_V1", {"packet": packet_identity, "ordinal": operation["ordinal"], "role": role}), "prior_ledger_head": LEDGER_HEAD, "grants_operational_content_access": False}
            requests.append({"operation_ordinal": operation["ordinal"], "purpose": operation["purpose"], "role": role, "challenge": {**challenge_core, "challenge_identity": seal("B2_PILOT14_SIGNING_CHALLENGE_V1", challenge_core)}, "signature_status": "UNSIGNED_AWAITING_SEPARATE_OWNER_ACTION"})
    require(len(requests) == 8, "eight signature requests")
    packet = {"schema_name": "batch2-development-pilot14-custodial-signing-packet-v1", "schema_version": "1.0.0", "packet_core": packet_core, "packet_identity": packet_identity, "signature_requests": requests, "status": "UNSIGNED", "signatures_present": 0, "source_ingested": False, "archive_written": False, "ledger_events_appended": 0, "proposition_sufficiency_evaluated": False, "downstream_planning_or_construction_performed": False}
    write("humor-mechanics-batch2-development-pilot14-preingestion-v1.json", prospective)
    write("humor-mechanics-batch2-development-pilot14-family-independence-v1.json", independence)
    write("humor-mechanics-batch2-development-pilot14-signing-packet-v1.json", packet)
