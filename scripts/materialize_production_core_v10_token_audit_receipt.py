...
EVIDENCE=ART/"production-core-v10-token-materialization-evidence.json"
EXPECTED_MAX={
...
def build(evidence_path:Path)->dict[str,object]:
    evidence=json.loads(evidence_path.read_bytes()); manifest=json.loads(MANIFEST.read_bytes())
    evidence_core=dict(evidence); claimed=evidence_core.pop("evidence_identity")
    if sha(canonical(evidence_core))!=claimed or evidence.get("status")!="TWO_MATERIALIZATIONS_BYTE_EXACT" or evidence.get("materializations")!=["A","B"]:
        raise ValueError("token materialization evidence mismatch")
    ...
    core={"schema":"pastila-production-core-v10-token-length-audit-receipt","schema_version":1,"status":"PASS_ZERO_BLOCKERS_TOKENIZER_ONLY_ZERO_MODEL_LOAD",
      ...
      "base_model_manifest_sha256":"f5a9327e40390c7f0cf93962d75abbbc5ae8da1fac48e1274092ac52cf88bf39","tokenizer_sha256":"2a00451398b3bb51d3c0fa3f4758c77061377ada35abbb7f5e1006be3aaced5c","rootfs_sha256":"9c8a26024a36a808bf077be880a214fcb3308233354e1824ae7ebc82d931f826","auditor_sha256":sha((ROOT/"scripts/audit_production_core_v10_token_lengths.py").read_bytes()),"token_materialization_evidence_identity":claimed,"maximum_sequence_tokens":3072,"observed":summary,"model_loaded":False,"inference_performed":False,"training_authorized":False,"training_performed":False,"qualification_execution_authorized":False,"qualification_attempt_consumed":False,"promotion_effect":False}
    return {**core,"receipt_identity":sha(canonical(core))}

def main()->int:
    parser=argparse.ArgumentParser();parser.add_argument("--evidence",type=Path,default=EVIDENCE);parser.add_argument("--output",type=Path,default=OUTPUT);args=parser.parse_args();value=build(args.evidence);raw=json.dumps(value,ensure_ascii=False,indent=2).encode()+b"\n"
