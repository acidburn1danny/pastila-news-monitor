"""Read-only closure and comparison audit for R6 holdout outputs."""
...
EXPECTED_ROWS = 12
FROZEN_ANSWER_KEY_SHA256 = "16e49c764c3aa0cdda3497c9905e8f158e4b38accacf536abbe9565526ad8f67"
RESPONSE_KEYS = ["schema", "schema_version", "case_id", "request_identity", "output_type", "outcome", "text", "claim_bindings", "abstention_code"]
SEMANTIC_MARKERS = {
    "PETITION_ATTRIBUTION": (("sustine", "sesizat", "afirma"), ("ar fi", "presupus"), ("respins", "respinge", "contesta"), ("nu a confirmat", "neconfirmat")),
    "DRAFT_REPORT_CONTEST": (("proiect", "preliminar"), ("ar fi", "ipoteza", "sugereaza"), ("contest", "observatii contrare"), ("nu este aprobat", "neaprobat")),
    "WITNESS_ATTRIBUTION": (("martor", "relatat"), ("ar fi", "presupus"), ("negat", "neaga"), ("nu a verificat", "neverificat")),
}
...
def lexical_case_passes(key: dict[str, object], parsed: dict[str, object]) -> bool:
    pattern = key.get("diagnostic_pattern")
    groups = SEMANTIC_MARKERS.get(pattern)
    text = parsed.get("text")
    if groups is None or not isinstance(text, str):
        raise ValueError("unsupported lexical diagnostic pattern")
    lowered = "".join(ch for ch in unicodedata.normalize("NFD", text.casefold()) if unicodedata.category(ch) != "Mn")
    return all(any(marker in lowered for marker in alternatives) for alternatives in groups)


def factual_bindings_valid(parsed: dict[str, object], expected: dict[str, object]) -> bool:
    bindings = parsed.get("claim_bindings")
    if not isinstance(bindings, list) or not 2 <= len(bindings) <= 3:
        return False
    allowed = {source for binding in expected["claim_bindings"] for source in binding["source_span_ids"]}
    for index, binding in enumerate(bindings, 1):
        if not isinstance(binding, dict) or list(binding) != ["claim_index", "source_span_ids"]:
            return False
        sources = binding["source_span_ids"]
        if (type(binding["claim_index"]) is not int or binding["claim_index"] != index
...
def audit(output: Path, answer_key: Path, candidate: str, adapter_sha256: str, requests_sha256: str, runner_sha256: str) -> dict[str, object]:
    ...
    if sha(answer_key.read_bytes()) != FROZEN_ANSWER_KEY_SHA256:
        raise ValueError("frozen answer-key identity mismatch")
    key_rows = [json.loads(line) for line in answer_key.read_bytes().splitlines()]
    keys = {row["example_id"]: row for row in key_rows}
    ...
    if len(key_rows) != EXPECTED_ROWS or len(keys) != EXPECTED_ROWS or len({row["example_id"] for row in rows}) != EXPECTED_ROWS:
        raise ValueError("evaluation closure mismatch")
    exact = structural = lexical = 0
    counts: Counter[str] = Counter()
    failures: Counter[str] = Counter()
    field_matches: Counter[str] = Counter()
    exact_by_class: Counter[str] = Counter()
    lexical_by_pattern: Counter[str] = Counter()
    lexical_misses: list[str] = []
    for index, row in enumerate(rows, 1):
        ...
        try:
            ...
            valid = (json.dumps(parsed, ensure_ascii=False, allow_nan=False, separators=(",", ":")).encode() == row["response"].encode()
                     ...
                     and parsed.get("schema") == "pastila-core-v2-structured-qualification-response"
                     and type(parsed.get("schema_version")) is int and parsed["schema_version"] == 2
                     and parsed.get("output_type") == "FACTUAL" and parsed.get("outcome") == "ANSWER"
                     and parsed.get("abstention_code") is None and isinstance(parsed.get("text"), str)
                     and 0 < len(parsed["text"]) <= 650 and factual_bindings_valid(parsed, expected_parsed)
                     and row["input_tokens"] <= 3072 and row["output_tokens"] <= 2048
                     and row["output_bytes"] == len(row["response"].encode()) <= 6268)
        except (ValueError, TypeError, KeyError):
            valid = False
        structural += valid
        lexical_pass = valid and lexical_case_passes(key, parsed)
        lexical += lexical_pass
        lexical_by_pattern[key["diagnostic_pattern"]] += lexical_pass
        if not lexical_pass:
            lexical_misses.append(row["example_id"])
    ...
    core = {
        "schema": "pastila-editor-core-targeted-r6-holdout-audit", "schema_version": 1,
        "candidate": candidate, "rows": EXPECTED_ROWS, "structural_passed": structural,
        "exact_target_passed": exact, "lexical_passed": lexical,
        "lexical_passed_by_pattern": dict(sorted(lexical_by_pattern.items())),
        "lexical_miss_cases": lexical_misses,
...
def compare(left: Path, right: Path, answer_key: Path) -> dict[str, object]:
    ...
    lexical_regressions: list[str] = []
    lexical_improvements: list[str] = []
    for index, key in enumerate(keys, 1):
        ...
        left_lexical = lexical_case_passes(key, json.loads(left_row["response"]))
        right_lexical = lexical_case_passes(key, json.loads(right_row["response"]))
        if left_lexical and not right_lexical:
            lexical_regressions.append(key["example_id"])
        if right_lexical and not left_lexical:
            lexical_improvements.append(key["example_id"])
    core = {
        "schema": "pastila-editor-core-targeted-r6-holdout-comparison", "schema_version": 1,
        ...
        "lexical_regressions": lexical_regressions, "lexical_improvements": lexical_improvements,
    }
