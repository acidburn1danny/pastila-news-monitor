...
if [[ $# -ne 16 ]]; then echo "usage: launcher ROOTFS_TAR MODEL ADAPTER PROMPT BATCH OUTPUT CANDIDATE GENERATION_ID QUALIFICATION_ID BATCH_SHA PROMPT_SHA RUNNER_SHA RUNNER DRIVER_ROOT DRIVER_MANIFEST_HELPER HELPER_SHA" >&2; exit 2; fi
...
readonly CANDIDATE="$7" GENERATION_ID="$8" QUALIFICATION_ID="$9"
readonly EXPECTED_BATCH_SHA256="${10}" EXPECTED_PROMPT_SHA256="${11}" EXPECTED_RUNNER_SHA256="${12}" RUNNER_INPUT="${13}"
readonly DRIVER_INPUT="${14}" HELPER_INPUT="${15}" EXPECTED_HELPER_SHA256="${16}"
readonly DRIVER_ROOT="$(realpath -e -- "$DRIVER_INPUT")" HELPER="$(realpath -e -- "$HELPER_INPUT")"
[[ ! -L "$DRIVER_INPUT" && ! -L "$HELPER_INPUT" && -d "$DRIVER_ROOT" && -f "$HELPER" && "$EXPECTED_HELPER_SHA256" =~ ^[0-9a-f]{64}$ ]] || exit 3
[[ "$DRIVER_ROOT" == /root/pf9-v15-wsl-drivers-snapshot && "$(stat -c '%d:%i' "$DRIVER_ROOT")" == 2096:21113 ]] || exit 3
[[ "$(sha256sum "$HELPER" | cut -d' ' -f1)" == "$EXPECTED_HELPER_SHA256" ]] || exit 3
...
[[ -z "$(find "$OUTPUT" -mindepth 1 -print -quit)" && "$GENERATION_ID" =~ ^[0-9a-f]{64}$ && "$QUALIFICATION_ID" =~ ^[0-9a-f]{64}$ && "$EXPECTED_BATCH_SHA256" =~ ^[0-9a-f]{64}$ && "$EXPECTED_PROMPT_SHA256" =~ ^[0-9a-f]{64}$ && "$EXPECTED_RUNNER_SHA256" =~ ^[0-9a-f]{64}$ ]] || exit 3
...
unshare --mount --net --pid --ipc --uts --fork bash -s -- \
  ...
  "$CANDIDATE" "$GENERATION_ID" "$QUALIFICATION_ID" "$EXPECTED_BATCH_SHA256" "$EXPECTED_PROMPT_SHA256" "$EXPECTED_RUNNER_SHA256" "$PARENT_NETNS" "$PARENT_PIDNS" "$DRIVER_ROOT" "$HELPER" "$EXPECTED_HELPER_SHA256" <<'CHILD'
set -euo pipefail
ROOTFS_TAR="$1"; MODEL="$2"; ADAPTER="$3"; PROMPT="$4"; BATCH="$5"; OUTPUT="$6"
RUNNER="$7"; CANDIDATE="$8"; GENERATION_ID="$9"; QUALIFICATION_ID="${10}"; BATCH_SHA256="${11}"; PROMPT_SHA256="${12}"; RUNNER_SHA256="${13}"
PARENT_NETNS="${14}"; PARENT_PIDNS="${15}"
DRIVER_ROOT="${16}"; HELPER="${17}"; HELPER_SHA256="${18}"
[[ "$DRIVER_ROOT" == /root/pf9-v15-wsl-drivers-snapshot && "$(stat -c '%d:%i' "$DRIVER_ROOT")" == 2096:21113 ]] || exit 4
...
[[ "$(sha256sum "$WORK/manifest.py" | cut -d' ' -f1)" == "$HELPER_SHA256" ]] || exit 4
python3 -B "$WORK/manifest.py" --root "$DRIVER_ROOT" >/dev/null || exit 4
mount --bind "$DRIVER_ROOT" "$ROOTFS/usr/lib/wsl/drivers"; mounted+=("$ROOTFS/usr/lib/wsl/drivers")
mount -o remount,bind,ro "$ROOTFS/usr/lib/wsl/drivers"
driver_opts="$(findmnt -no OPTIONS --target "$ROOTFS/usr/lib/wsl/drivers")"
[[ ",$driver_opts," == *,ro,* ]] || { echo "driver snapshot mount is writable" >&2; exit 5; }
