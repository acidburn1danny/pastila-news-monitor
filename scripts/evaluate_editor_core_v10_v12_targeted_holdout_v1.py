"""Deterministic inference-only runner for the targeted independent holdout."""
...
def run(model_path: Path, adapter_path: Path, requests_path: Path, output: Path, candidate: str) -> dict[str, object]:
    ...
    request_raw = requests_path.read_bytes()
    if sha(request_raw) != os.environ["REQUESTS_SHA256"] or sha(Path(__file__).read_bytes()) != os.environ["RUNNER_SHA256"]:
    ...
    for index, row in enumerate(rows, 1):
        ...
        observation = {
            ...
            "within_byte_ceiling": len(raw) <= MAX_OUTPUT_BYTES,
    ...
    core = {
        "schema": "pastila-editor-core-targeted-holdout-inference-receipt",
        "schema_version": 1,
        "candidate": candidate,
        "adapter_sha256": os.environ["ADAPTER_SHA256"],
        "requests_sha256": os.environ["REQUESTS_SHA256"],
        "runner_sha256": os.environ["RUNNER_SHA256"],
        "rows": len(observations),
        "terminal_eos": sum(row["terminal_eos"] for row in observations),
...
def fixture_smoke(requests: list[dict[str, object]]) -> dict[str, object]:
    ...
    core = {"schema": "pastila-editor-core-targeted-holdout-fixture-smoke", "schema_version": 1, "rows": 2, "model_loaded": False, "inference_performed": False, "training_performed": False}
