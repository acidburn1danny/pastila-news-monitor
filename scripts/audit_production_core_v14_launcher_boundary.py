"""Read-only Ed25519 and source audit of the V14 launcher boundary."""
...
import materialize_production_core_v14_launcher_boundary as launcher_boundary
...
def audit(output: Path = launcher_boundary.OUTPUT) -> dict[str, str]:
    ...
    if (recorded != launcher_boundary.digest(launcher_boundary.canonical(core))
            or boundary.get("schema") != "pastila-production-core-v14-preconsumption-launcher-boundary"
            or boundary.get("schema_version") != 1
            or boundary.get("status") != "SIGNED_PREFLIGHT_ONLY_ZERO_ATTEMPTS"
            or boundary.get("v14_authority_identity") != launcher_boundary.AUTHORITY_ID
