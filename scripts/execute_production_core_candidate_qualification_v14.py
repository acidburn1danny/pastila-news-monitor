"""WSL-native V14 adapter around byte-pinned execution mechanics."""
...
import audit_production_core_v14_launcher_boundary as audit_launcher
import preflight_production_core_candidate_qualification_v14 as preflight
...
def projected_mechanics() -> str:
    ...
    if source.count(old_key) != 1:
        raise ValueError("V14 executor source-key witness mismatch")
    source = source.replace(old_key, '"scripts/execute_production_core_candidate_qualification_v14.py"')
    old_except = "except OSError, ValueError, json.JSONDecodeError, SystemExit:"
    if source.count(old_except) != 1:
        raise ValueError("V14 executor syntax repair witness mismatch")
    ...
    if source.count(old_independence) != 1:
        raise ValueError("V14 shared-rootfs projection witness mismatch")
    source = source.replace(old_independence, new_independence)
    old_generation_arg = '            GENERATION_IDENTITY,\n            str(batch["batch_sha256"]),'
    new_generation_arg = '            GENERATION_IDENTITY,\n            QUALIFICATION_IDENTITY,\n            str(batch["batch_sha256"]),'
    if source.count(old_generation_arg) != 1:
        raise ValueError("V14 qualification argument witness mismatch")
    return source.replace(old_generation_arg, new_generation_arg)
...
def object_authority(path: str, kind: str, helper: bytes) -> dict:
    ...
    if tuple(value) != ("physical_identity", "content_identity"):
        raise ValueError("V14 object authority response rejected")
    return value


def linux_path(path: Path) -> str:
    if path.is_symlink() or not path.exists() or not path.is_absolute():
        raise ValueError("V14 Linux path rejected")
    return str(path)
...
def validate_and_map(generation: dict, requests: dict, candidates: dict, qualification: dict):
    ...
    if tuple(adapters) != ("pastila-editor-core-v1.1-json-successor-v10", "pastila-editor-core-v1.2-json-successor-v10"):
        raise ValueError("V14 candidate manifest mapping mismatch")
...
def terminal_validator(snapshots: dict[str, bytes]) -> None:
    if tuple(snapshots) != NAMES:
        raise ValueError("V14 public snapshot set mismatch")
    for name, raw in snapshots.items():
        committed = subprocess.check_output(["git", "show", f"{preflight.COMMIT}:docs/artifacts/{name}"], cwd=ROOT)
        if raw != committed:
            raise ValueError(f"V14 public snapshot drift: {name}")
...
def build_namespace() -> dict:
    ...
    if any(name not in source_hashes for name in SOURCE_KEYS):
        raise ValueError("V14 executable source closure incomplete")
    ...
    for name in ("GENERATION_IDENTITY", "QUALIFICATION_IDENTITY", "MATRIX_ROWS", "build_attempt", "build_terminal_failure", "canonical",
    ...
    namespace.update({
        ...
        "LAUNCHER": ROOT / "scripts/run_production_core_candidate_qualification_v14.sh",
        "NAMES": NAMES,
        "PROMPTS": PROMPTS,
