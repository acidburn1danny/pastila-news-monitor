"""Verify the published bounded v3.1 development-inference authority."""
...
def published(repo: Path, path: Path) -> bool:
    relative=path.resolve().relative_to(repo.resolve()).as_posix()
    try: raw=subprocess.check_output(["git","-c",f"safe.directory={repo}","-C",str(repo),"show",f"HEAD:{relative}"])
    except subprocess.CalledProcessError: return False
    return hashlib.sha256(raw).digest()==hashlib.sha256(path.read_bytes()).digest()
def verify(authority:Path,candidate:str,model:Path,adapter:Path,requests:Path,worker:Path,route:Path,repo:Path)->dict:
    ...
    if value.get("authority_identity")!=identity or value.get("schema")!="editor-core-bridge-causal-development-inference-structural-successor-authority": raise ValueError("authority identity")
    ...
    if not all(published(repo,path) for path in (authority,worker,route,repo/value["audit_path"],
                                                  repo/value["supervisor_path"],repo/value["constraint_path"],repo/value["preflight_path"],repo/value["design_path"],Path(__file__))): raise ValueError("published source closure")
    if subprocess.run(["git","-c",f"safe.directory={repo}","-C",str(repo),"merge-base","--is-ancestor",value["implementation_parent_commit"],"HEAD"]).returncode: raise ValueError("implementation ancestry")
    ...
    if (flat(model)!=value["model_sha256"] or sha(requests)!=value["development_requests_sha256"]
        or sha(worker)!=value["worker_sha256"] or sha(route)!=value["route_sha256"]
        or sha(repo/value["audit_path"])!=value["audit_sha256"]
        or sha(repo/value["supervisor_path"])!=value["supervisor_sha256"]
        or sha(repo/value["constraint_path"])!=value["constraint_sha256"]
        or sha(repo/value["preflight_path"])!=value["preflight_sha256"]
        or sha(repo/value["design_path"])!=value["design_sha256"]): raise ValueError("source binding")
