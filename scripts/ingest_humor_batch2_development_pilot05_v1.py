"""Verify all Pilot 05 custodial proofs and atomically materialize ingestion."""
...
FREEZE_COMMIT = "cd80affa6d8411954b472aa9828a764de11d24be"
HELPER_COMMIT = "1aab08528b2cd4e2c4485457a99d2e0704a87ae9"
PRE_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot05-preingestion-v1.json"
PACKET_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot05-signing-packet-v1.json"
...
PRIOR_LEDGER_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot04-ingestion-v1/access-ledger-segment.json"
SOURCE = ROOT / "owner-source-pilot05-v1.txt"
DECLARATION = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot05-owner-declaration-canonical-v1.json"
DESTINATION = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot05-ingestion-v1"
...
HANDOFF = Path.home() / "Pastila-Owner-Handoff" / "Batch2-Development-Pilot05-v1"
...
def verify_signature(public_pem: str, algorithm: str, message: bytes, signature: bytes) -> bool:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot05-verify-") as temp:
        folder = Path(temp)
...
def spki_fingerprint(public_pem: str) -> str:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot05-spki-") as temp:
...
def verify(responses: Path) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    if packet["packet_identity"] != seal("B2_DEVELOPMENT_PILOT05_SIGNING_PACKET_V1", packet["packet_core"]):
    ...
    paths = sorted(responses.glob("*.pilot05-response.json"))
    ...
    for path in paths:
        ...
        if seal("B2_PILOT05_SIGNING_CHALLENGE_V1", core) != identity:
        ...
        if not (
            ...
            and challenge["domain"] == "PASTILA_BATCH2_DEVELOPMENT_PILOT05_PREINGESTION_V1"
...
def materialize(pre: dict[str, Any], packet: dict[str, Any], verified: list[dict[str, Any]], prior_ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    source, declaration = SOURCE.read_bytes(), DECLARATION.read_bytes()
    response_set_identity = seal("B2_DEVELOPMENT_PILOT05_VERIFIED_RESPONSE_SET_V1", verified)
    previous = prior_ledger["final_ledger_head"]
    entries = []
    for sequence, operation in enumerate(packet["packet_core"]["operations"], start=43):
        ...
        entry = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                 ...
                 "event": "DEVELOPMENT_PILOT05_ATOMIC_INGESTION_OPERATION_ADMITTED",
    ...
    completion = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                  "entry_sequence": 49, "previous_entry_hash": previous,
                  "event": "DEVELOPMENT_PILOT05_ATOMIC_INGESTION_COMPLETED",
    ...
    ledger = {"schema_name": "batch2-development-pilot05-access-ledger-segment-v1", "schema_version": "1.0.0",
              "previous_ledger_head": prior_ledger["final_ledger_head"], "entries": entries,
              "final_ledger_head": completion["entry_hash"]}
    ledger["ledger_segment_identity"] = seal("B2_DEVELOPMENT_PILOT05_ACCESS_LEDGER_SEGMENT_V1", ledger)
    ...
    archive_receipt = {"schema_name": "batch2-development-pilot05-immutable-archive-receipt-v1", "schema_version": "1.0.0",
                       "immutable_archive_commitment": pre["immutable_archive_commitment"],
    ...
    archive_receipt["archive_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT05_ARCHIVE_RECEIPT_V1", archive_receipt)
    evidence = {"schema_name": "batch2-development-pilot05-custodial-verification-v1", "schema_version": "1.0.0",
    ...
    evidence["verification_identity"] = seal("B2_DEVELOPMENT_PILOT05_CUSTODIAL_VERIFICATION_V1", evidence)
    receipt = {"schema_name": "batch2-development-pilot05-ingestion-receipt-v1", "schema_version": "1.0.0",
    ...
    receipt["ingestion_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT05_INGESTION_RECEIPT_V1", receipt)
    temporary = Path(tempfile.mkdtemp(prefix=".pilot05-ingestion-", dir=DESTINATION.parent))
    try:
