...
if [[ $# -ne 13 ]]; then echo "usage: launcher ROOTFS_TAR MODEL ADAPTER PROMPT BATCH OUTPUT CANDIDATE GENERATION_ID QUALIFICATION_ID BATCH_SHA PROMPT_SHA RUNNER_SHA RUNNER" >&2; exit 2; fi
...
unshare --mount --net --pid --ipc --uts --fork bash -s -- \
  ...
  "$CANDIDATE" "$GENERATION_ID" "$QUALIFICATION_ID" "$EXPECTED_BATCH_SHA256" "$EXPECTED_PROMPT_SHA256" "$EXPECTED_RUNNER_SHA256" "$PARENT_NETNS" "$PARENT_PIDNS" <<'CHILD'
set -euo pipefail
