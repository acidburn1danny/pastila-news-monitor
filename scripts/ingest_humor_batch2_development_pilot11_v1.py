"""Verify Pilot 11 custodial proofs and atomically materialize ingestion."""
...
HELPER_COMMIT = "c37ab03bec1e645d856185e75deaeb5063e30708"
PRE_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot11-preingestion-v1.json"
PACKET_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot11-signing-packet-v1.json"
REG_PATH = "docs/artifacts/humor-mechanics-batch2-custodial-public-key-registration-v1.json"
PRIOR_LEDGER_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot10-ingestion-v1/access-ledger-segment.json"
SOURCE = ROOT / "owner-source-pilot11-v1.txt"
DECLARATION = ROOT / "owner-declaration-pilot11-v1.json"
DESTINATION = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot11-ingestion-v1"
SOURCE_SHA = "cdf1901941057914cb7b22ac1233771773e2f15bd1671bcc47e2d17d123e2bd9"
DECLARATION_SHA = "6fdb4ca1cac39f6b4cf4ae9614163d0641695608568bebc4e582322190a3ed21"
PACKET_ID = "9148537ef0d1788bc66e02a1e916c4e2e9e4ff6c4ba745a508bc025c5e73d95e"
HANDOFF = Path.home() / "Pastila-Owner-Handoff" / "Batch2-Development-Pilot11-v1"
...
def verify_signature(public_pem: str, algorithm: str, message: bytes, signature: bytes) -> bool:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot11-verify-") as temp:
        folder = Path(temp)
...
def spki_fingerprint(public_pem: str) -> str:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot11-spki-") as temp:
...
def verify(responses: Path) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    if head != HELPER_COMMIT:
        raise SystemExit("HEAD differs from authorized Pilot 11 helper commit")
    ...
    if packet["packet_identity"] != PACKET_ID or seal("B2_DEVELOPMENT_PILOT11_SIGNING_PACKET_V1", packet["packet_core"]) != PACKET_ID:
        raise SystemExit("packet identity/seal")
    if any((packet["proposition_sufficiency_evaluated"], packet["constructor_semantic_plan_release_or_invocation_performed"],
            packet["realization_candidate_emission_or_semantic_edge_validation_performed"], packet["fragment_collision_evaluation_performed"])):
    ...
    paths = sorted(responses.glob("*.pilot11-response.json"))
    ...
    for path in paths:
        ...
        if seal("B2_PILOT11_SIGNING_CHALLENGE_V1", challenge_core) != challenge_identity:
        ...
        if not (response["packet_identity"] == PACKET_ID and response["purpose"] == request["purpose"] == challenge["purpose"]
                ...
                and challenge["domain"] == "PASTILA_BATCH2_DEVELOPMENT_PILOT11_PREINGESTION_V1"
...
def materialize(pre: dict[str, Any], packet: dict[str, Any], verified: list[dict[str, Any]], prior: dict[str, Any]) -> dict[str, Any]:
    ...
    source, declaration = SOURCE.read_bytes(), DECLARATION.read_bytes()
    response_set = seal("B2_DEVELOPMENT_PILOT11_VERIFIED_RESPONSE_SET_V1", verified)
    previous, entries = prior["final_ledger_head"], []
    for sequence, operation in enumerate(packet["packet_core"]["operations"], start=85):
        entry = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
            ...
            "event": "DEVELOPMENT_PILOT11_ATOMIC_INGESTION_OPERATION_ADMITTED", "packet_identity": PACKET_ID,
    ...
    completion = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
        "entry_sequence": 91, "previous_entry_hash": previous, "event": "DEVELOPMENT_PILOT11_ATOMIC_INGESTION_COMPLETED",
    ...
    ledger = {"schema_name": "batch2-development-pilot11-access-ledger-segment-v1", "schema_version": "1.0.0",
        "previous_ledger_head": prior["final_ledger_head"], "entries": entries, "final_ledger_head": completion["entry_hash"]}
    ledger["ledger_segment_identity"] = seal("B2_DEVELOPMENT_PILOT11_ACCESS_LEDGER_SEGMENT_V1", ledger)
    ...
    archive = {"schema_name": "batch2-development-pilot11-immutable-archive-receipt-v1", "schema_version": "1.0.0",
    ...
    archive["archive_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT11_ARCHIVE_RECEIPT_V1", archive)
    evidence = {"schema_name": "batch2-development-pilot11-custodial-verification-v1", "schema_version": "1.0.0",
    ...
    evidence["verification_identity"] = seal("B2_DEVELOPMENT_PILOT11_CUSTODIAL_VERIFICATION_V1", evidence)
    receipt = {"schema_name": "batch2-development-pilot11-ingestion-receipt-v1", "schema_version": "1.0.0",
        ...
        "constructor_v5_3_compatibility_or_semantic_plan_evaluated": False, "verification_identity": evidence["verification_identity"],
    ...
    receipt["ingestion_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT11_INGESTION_RECEIPT_V1", receipt)
    temporary = Path(tempfile.mkdtemp(prefix=".pilot11-ingestion-", dir=DESTINATION.parent))
    try:
