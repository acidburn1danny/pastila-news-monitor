...
ARMS = {
    "T1_S0_EXISTING_CONTROL": ("T1", "5e-7"),
    "T1_S0_REPLAY_PROTECTED": ("T1", "5e-7"),
}
SEEDS = (161803, 271828, 314159)
EXPECTED_PROTOCOL = "9b6276e9847978a538141122d600b77f69f6920a286455b9da6d9c6cf415284f"
EXPECTED_PACK = "2676266a823346a8a361bf009f2c3dae0ed2226bb86731e59e8c4a735810b588"
...
def run_slot(model_path: Path, parent_path: Path, corpus_path: Path, learning_signal_path: Path,
    ...
    if os.environ.get("T1S0_REPLAY_PROTECTED_REAL_RUN_AUTHORIZED")!="1": raise RuntimeError("real runs are not authorized")
    ...
    for row in corpus:
        ...
        mapping=real_chat_token_map(tokenizer,messages,annotations[row["example_id"]]["critical_spans"])
        learning_mapping=real_chat_token_map(tokenizer,messages,learning[row["example_id"]]["critical_spans"])
        if mapping["input_ids"] != learning_mapping["input_ids"]: raise ValueError("learning/measurement token drift")
        tokens=mapping["input_ids"]; start=mapping["assistant_token_start"]
        if not tokens or tokens[-1]!=tokenizer.eos_token_id or len(tokens)>3072: raise ValueError("token sequence closure")
        measured=set(); weighted=set()
        for span in mapping["mapped_spans"]: measured.update(range(span["token_start"],span["token_end_exclusive"]))
        for span in learning_mapping["mapped_spans"]: weighted.update(range(span["token_start"],span["token_end_exclusive"]))
        prepared.append((tokens,start,measured,weighted))

    def probe() -> dict:
        model.eval(); full=[]; critical=[]
        with torch.no_grad():
            for tokens,start,critical_ids,_ in prepared:
