"""Offline shadow qualification under the bound V10 execution contract."""
...
def flat_manifest(root:Path)->str:
 ...
 for path in sorted(root.iterdir(),key=lambda p:p.name.encode()):
  if path.is_symlink() or not path.is_file():raise SystemExit("shadow object closure mismatch")
...
def repeated_ngram_ceiling(text:str,width:int=8)->int:
 words=text.split();return 1 if len(words)<width else max(Counter(tuple(words[i:i+width]) for i in range(len(words)-width+1)).values())
...
def main()->int:
 if len(sys.argv)!=9 or set(os.environ)!=EXPECTED_ENV:raise SystemExit("shadow invocation mismatch")
 ...
 if materialization not in {"A","B"} or any(output.iterdir()):raise SystemExit("shadow output mismatch")
 ...
 if any(value!=os.environ[key] for value,key in bindings):raise SystemExit("shadow authority binding mismatch")
 ...
 if len(rows)!=240 or any(row.get("split")!="SHADOW_QUALIFICATION" for row in rows):raise SystemExit("shadow corpus closure mismatch")
 rendered=[]
 for row in rows:
  stored=row["messages"][:2];user_prompt=stored[1]["content"];messages=list(contract.render_shadow_qualification_messages(candidate=candidate,editorial_prompt=editorial,user_prompt=user_prompt))
  if messages!=stored or contract.assert_phase_equivalence(candidate=candidate,editorial_prompt=editorial,user_prompt=user_prompt)!=contract.projection_identity(messages):raise SystemExit("V10 shadow projection mismatch")
 ...
 for index,(row,messages) in enumerate(zip(rows,rendered,strict=True),1):
  ...
  if input_tokens>policy["maximum_input_tokens"]:raise SystemExit("shadow input token ceiling exceeded")
  ...
  observation={"index":index,"example_id":row["example_id"],"projection_identity":contract.projection_identity(messages),"terminal_eos":terminal_eos,"canonical_json":canonical_json,"nfc":unicodedata.normalize("NFC",text)==text,"within_byte_ceiling":len(raw)<=policy["maximum_utf8_bytes"],"exact_target":text==row["messages"][2]["content"],"input_tokens":input_tokens,"output_tokens":len(tokens),"output_bytes":len(raw),"repeated_8gram_ceiling":repeated_ngram_ceiling(text),"output_sha256":sha(raw)}
  observations.append(observation);(output/f"{index:03d}.json").write_bytes(canonical(observation))
 passed=all(r["terminal_eos"] and r["canonical_json"] and r["nfc"] and r["within_byte_ceiling"] and r["repeated_8gram_ceiling"]<=3 for r in observations)
 core={"schema":"pastila-production-core-shadow-qualification-gate","schema_version":10,"materialization":materialization,"candidate":candidate,"adapter_sha256":os.environ["ADAPTER_SHA256"],"shadow_corpus_sha256":os.environ["DEVELOPMENT_SHA256"],"contract_sha256":os.environ["CONTRACT_SHA256"],"editorial_sha256":os.environ["EDITORIAL_SHA256"],"launcher_sha256":os.environ["LAUNCHER_SHA256"],"runner_sha256":os.environ["RUNNER_SHA256"],"rows":240,"terminal_eos_passed":sum(r["terminal_eos"] for r in observations),"canonical_json_passed":sum(r["canonical_json"] for r in observations),"nfc_passed":sum(r["nfc"] for r in observations),"byte_ceiling_passed":sum(r["within_byte_ceiling"] for r in observations),"exact_target_passed":sum(r["exact_target"] for r in observations),"anti_repetition_passed":sum(r["repeated_8gram_ceiling"]<=3 for r in observations),"status":"PASS" if passed else "FAIL_CLOSED","qualification_rows_used":0,"qualification_attempt_consumed":False,"adjudication_performed":False,"promotion_effect":False}
 receipt={**core,"gate_identity":sha(canonical(core))};(output/"gate-receipt.json").write_bytes(canonical(receipt))
 if not passed:raise SystemExit("shadow gate failed closed")
 return 0
