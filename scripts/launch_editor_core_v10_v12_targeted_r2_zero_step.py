"""Zero-step launcher validation for the Editor Core targeted R2 round.
...
ACTIVE_COMMIT = "c68cc2c8c6a7797790679c8790d65a4e5ae0b802"
ACTIVE_TREE = "f58094bdc2a1efe3681a42d70d6ee8b210c54dd6"
...
def validate_public_inputs() -> tuple[dict[str, object], dict[str, object], dict[str, object]]:
    result = json.loads((ARTIFACTS / "editor-core-v10-v12-targeted-r1-result.json").read_bytes())
    result_identity = result.pop("result_identity")
    if result_identity != hashlib.sha256(canonical(result)).hexdigest():
        raise ValueError("R1 result identity mismatch")
    if result["selection"]["selected_adapter_identity"] != PARENT_ADAPTER or result["training"]["checkpoint_8_identity"] != PARENT_CHECKPOINT:
        raise ValueError("active parent selection mismatch")
    ...
    audit_module = load_script(
        ROOT / "scripts" / "audit_editor_core_v10_v12_targeted_r2_failure_mining_pack.py",
        "targeted_r2_audit",
    )
...
def validate_parent(checkpoint: Path) -> dict[str, object]:
    ...
    if identity_value != PARENT_CHECKPOINT or receipt.get("model_sha256") != BASE_MODEL or receipt.get("optimizer_steps") != 8:
...
def zero_step(model: Path, checkpoint: Path, rootfs: Path, snapshot: Path, output: Path) -> dict[str, object]:
    ...
    core = {
        "schema": "pastila-editor-core-targeted-r2-zero-step-receipt",
        "schema_version": 2,
