...
def verify(*, project_root: Path) -> dict[str, object]:
    ...
    if actual_names not in (set(expected), allowed_names):
        raise RuntimeError("CASE01_V1_2_ISSUANCE_PACKET_FILE_SET_MISMATCH")
    for name, raw in expected.items():
        if (packet_root / name).read_bytes() != raw:
            raise RuntimeError(f"CASE01_V1_2_ISSUANCE_PACKET_BYTE_DRIFT:{name}")
    ...
    if (candidate["receipt_status"] != "UNISSUED"
        ...
        raise RuntimeError("CASE01_V1_2_ISSUANCE_PACKET_AUTHORITY_STATUS_INVALID")
    if (root / EVIDENCE_RELATIVE).exists() or (root / EVIDENCE_RELATIVE).is_symlink():
        raise RuntimeError("CASE01_V1_2_ISSUANCE_PACKET_EVIDENCE_ROOT_NOT_EXCLUSIVE")
    ...
    if parsed.authority_receipt_identity != candidate["proposed_receipt_identity"]:
        raise RuntimeError("CASE01_V1_2_ISSUANCE_PACKET_PROPOSED_RECEIPT_INVALID")
    return manifest
