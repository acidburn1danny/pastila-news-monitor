"""Owner-operated signer for the frozen Pilot 03 unsigned packet.
...
REQUEST = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot03-signing-packet-v1.json"
...
def sign_all(packet: dict[str, Any], key_dir: Path, response_dir: Path) -> None:
    ...
    for item in packet["signature_requests"]:
        ...
        output = response_dir / f"{item['operation_ordinal']:02d}-{stem}.pilot03-response.json"
        if output.exists():
        ...
        with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot03-") as temporary:
        ...
        response = {
            "schema_name": "batch2-development-pilot03-custodial-signature-response-v1",
            ...
            "algorithm": algorithm,
            "signature": {"encoding": "BASE64", "value": signature_b64},
