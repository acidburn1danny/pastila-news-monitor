"""Offline development-only structural gate for an audited successor adapter."""
...
def main() -> int:
    ...
    if sha(Path(__file__).read_bytes()) != os.environ["RUNNER_SHA256"]:
        raise SystemExit("development runner identity mismatch")
    rows = [json.loads(line) for line in probes.read_text("utf-8").splitlines()]
    expected_rows = len(rows) if len(rows) in {48, 72} else None
    if expected_rows is None or any(row.get("split") != "DEVELOPMENT" for row in rows):
    ...
    core = {
        "schema": "pastila-production-core-successor-development-gate",
        "schema_version": 9 if expected_rows == 72 else 5,
