"""Fail-closed closure and contamination audit for targeted R3."""
...
def audit() -> dict:
    ...
    if tuple(map(len, (new, replay, training, holdout, keys))) != (24, 24, 48, 12, 12) or training != new + replay:
        raise ValueError("cardinality/projection mismatch")
    if set(row["failure_class"] for row in new + holdout) != {"EPISTEMIC_CALIBRATION"}:
        raise ValueError("failure-class scope mismatch")
    if Counter(row["diagnostic_pattern"] for row in new) != Counter({"ATTRIBUTION_DENIAL_FINALITY": 12, "PRELIMINARY_CONTEST_PENDING": 12}):
        raise ValueError("training pattern balance mismatch")
    if Counter(row["diagnostic_pattern"] for row in holdout) != Counter({"ATTRIBUTION_DENIAL_FINALITY": 6, "PRELIMINARY_CONTEST_PENDING": 6}):
    ...
    r2_holdout_ids = {row["example_id"] for row in [json.loads(line) for line in (ART / "editor-core-v10-v12-targeted-continuation-r2-holdout-requests.jsonl").read_bytes().splitlines()]}
    if r2_holdout_ids & {row["example_id"] for row in training}:
        raise ValueError("R2 holdout used for training")
    r2_targets = {row["assistant_target"] for row in [json.loads(line) for line in (ART / "editor-core-v10-v12-targeted-continuation-r2-holdout-answer-key.jsonl").read_bytes().splitlines()]}
    if r2_targets & {row["messages"][2]["content"] for row in training if len(row["messages"]) == 3}:
        raise ValueError("R2 holdout target reuse")
    references = []
    ...
    return {"verdict": "PASS", "blockers": 0, "manifest_identity": manifest["manifest_identity"], "training_config_identity": config["training_config_identity"], "new_rows": 24, "replay_rows": 24, "holdout_rows": 12, "replay_provenance": replay_provenance, "reference_rows_checked": len(references), "exact_contamination_matches": len(exact), "maximum_external_jaccard_5gram": maximum, "maximum_train_holdout_jaccard_5gram": internal, "r2_holdout_training_use": False, "r2_holdout_targets_reused": False, "r4_evidence_used": False, "token_audit_identity": token["audit_identity"], "maximum_training_sequence_tokens": token["maximum_training_sequence_tokens"], "training_performed": False}
