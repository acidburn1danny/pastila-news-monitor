...
def verify(authority:Path,candidate:str,model:Path,adapter:Path,requests:Path,worker:Path,route:Path,repo:Path)->dict:
    ...
    if value.get("authority_identity")!=identity or value.get("schema")!="editor-core-bridge-causal-development-inference-successor-authority": raise ValueError("authority identity")
    ...
    if not all(published(repo,path) for path in (authority,worker,route,repo/value["audit_path"],
                                                  repo/value["supervisor_path"],repo/value["design_path"],Path(__file__))): raise ValueError("published source closure")
