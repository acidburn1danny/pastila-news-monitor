"""Read-only verification of immutable Pilot 08 ingestion artifacts."""
...
DIR = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot08-ingestion-v1"
...
def main() -> None:
    ...
    require(hashlib.sha256(source).hexdigest() == "d2a71300c1d1832f68132e4b824714ec0bc51beecf26f750146befb00a26712a", "source")
    require(hashlib.sha256(rights).hexdigest() == "7a7da131c60d7a2e1aece6804edd5c7256dca15e534cf3cac3205ebdf39b74b4", "rights")
    require(receipt["ingestion_receipt_identity"] == seal("B2_DEVELOPMENT_PILOT08_INGESTION_RECEIPT_V1", {k: v for k, v in receipt.items() if k != "ingestion_receipt_identity"}), "receipt")
    require(archive["archive_receipt_identity"] == seal("B2_DEVELOPMENT_PILOT08_ARCHIVE_RECEIPT_V1", {k: v for k, v in archive.items() if k != "archive_receipt_identity"}), "archive")
    require(evidence["verification_identity"] == seal("B2_DEVELOPMENT_PILOT08_CUSTODIAL_VERIFICATION_V1", {k: v for k, v in evidence.items() if k != "verification_identity"}), "evidence")
    require(ledger["ledger_segment_identity"] == seal("B2_DEVELOPMENT_PILOT08_ACCESS_LEDGER_SEGMENT_V1", {k: v for k, v in ledger.items() if k != "ledger_segment_identity"}), "ledger")
    ...
    require([p["proposition_id"] for p in propositions] == [f"P{i}" for i in range(1, 8)], "propositions")
    ...
    require(package["source_package_identity"] == receipt["source_package_identity"] == "2fd6e1cee4fe69136d976bd988c004de354c5c3cfee553d92f6395ac464fe64f", "package")
    ...
    print(json.dumps({"verdict": receipt["terminal_verdict"], "signature_verification": evidence["verification_result"],
                      "seven_proposition_bindings": "PASS", "source_git_blob_oid_sha1": blob,
