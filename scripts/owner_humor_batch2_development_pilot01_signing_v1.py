"""Owner-operated signer for Development Pilot 01 pre-ingestion requests.

The assistant may run ``inspect`` but must never run ``sign-all``. Private keys
and public responses must remain outside the repository.
"""
...
REQUEST = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot01-signing-packet-v1.json"
...
def inspect(packet: dict[str, Any]) -> None:
    ...
    for item in packet["signature_requests"]:
        ...
        rows.append({
            ...
            "signer_role": item["role"], "challenge_identity": challenge["challenge_identity"],
            "canonical_challenge_sha256": hashlib.sha256(canonical(challenge)).hexdigest(),
            "nonce": challenge["nonce"], "prior_ledger_head": challenge["prior_ledger_head"],
        })
...
def sign_all(packet: dict[str, Any], key_dir: Path, response_dir: Path) -> None:
    ...
    for item in packet["signature_requests"]:
        ...
        output = response_dir / f"{item['operation_ordinal']:02d}-{stem}.pilot01-response.json"
        if output.exists():
        ...
        with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot01-") as tmp:
        ...
        response = {
            "schema_name": "batch2-development-pilot01-custodial-signature-response-v1",
            "schema_version": "1.0.0", "packet_identity": packet["packet_identity"],
            "operation_ordinal": item["operation_ordinal"], "purpose": item["purpose"],
            ...
            "algorithm": algorithm, "signature": {"encoding": "BASE64", "value": signature_b64},
...
def main() -> None:
    ...
    packet = json.loads(args.request.read_text(encoding="utf-8"))
