"""Fail-closed closure and contamination audit for targeted R5."""
...
def audit() -> dict:
    ...
    if (config["new_training_rows"], config["replay_rows"], config["independent_holdout_rows"],
       ...
       (12, 24, 12, "0.0000005", 6, 6, 3072, False, False):
        raise ValueError("training recipe mismatch")
    if manifest["rows"] != {"new_train": 12, "replay": 24, "holdout": 12} or manifest["replay_sources"] != {"v10_v1_2": 8, "targeted_r1": 8, "targeted_r2": 8}:
    ...
    if tuple(map(len, (new, replay, training, holdout, keys))) != (12, 24, 36, 12, 12) or training != new + replay:
    ...
    if Counter(row["diagnostic_pattern"] for row in new) != Counter({"ATTRIBUTION_DENIAL_FINALITY": 6, "PRELIMINARY_CONTEST_PENDING": 6}):
    ...
    return {"verdict": "PASS", "blockers": 0, "manifest_identity": manifest["manifest_identity"], "training_config_identity": config["training_config_identity"], "new_rows": 12, "replay_rows": 24, "holdout_rows": 12, "replay_provenance": replay_provenance, "reference_rows_checked": len(references), "exact_contamination_matches": len(exact), "maximum_external_jaccard_5gram": maximum, "maximum_train_holdout_jaccard_5gram": internal, "frozen_holdout_training_use": False, "frozen_holdout_targets_reused": False, "rejected_r3_r4_adapter_training_use": False, "r4_diagnostics_used_for_design_only": True, "token_audit_identity": token["audit_identity"], "maximum_training_sequence_tokens": token["maximum_training_sequence_tokens"], "training_performed": False}
