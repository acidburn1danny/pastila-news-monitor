...
MODE="$8"; [[ "$MODE" == SMOKE || "$MODE" == CHECKPOINT_SMOKE || "$MODE" == BATCH_SMOKE || "$MODE" == FULL ]] || exit 3
[[ "$(id -u)" == 0 && -f "$ROOTFS_TAR" && -d "$MODEL" && -d "$PREDECESSOR" && -f "$CORPUS" && -f "$CONFIG" && -f "$TRAINER" ]] || exit 3
mkdir -p "$OUTPUT"
[[ -z "$(find "$OUTPUT" -mindepth 1 -maxdepth 1 ! -name training-progress.jsonl ! -name training-checkpoints -print -quit)" ]] || exit 3
[[ ! -L "$OUTPUT/training-progress.jsonl" && ! -L "$OUTPUT/training-checkpoints" ]] || exit 3
...
WORK="$(mktemp -d /tmp/pcs-train.XXXXXXXX)"; ROOTFS="$WORK/rootfs"; mkdir "$ROOTFS"; mounted=(); child_pid=""
cleanup() { local i; for ((i=${#mounted[@]}-1; i>=0; i--)); do umount -l -- "${mounted[$i]}" 2>/dev/null || true; done; rm -rf -- "$WORK"; }
terminate() {
  trap - INT TERM
  if [[ -n "$child_pid" ]] && kill -0 "$child_pid" 2>/dev/null; then
    kill -TERM -- "-$child_pid" 2>/dev/null || true
    for _ in {1..30}; do kill -0 "$child_pid" 2>/dev/null || break; sleep 1; done
...
setsid unshare --kill-child=KILL --mount --net --pid --ipc --uts --fork bash -c '
set -euo pipefail
tar -xf "$1" -C "$2"
