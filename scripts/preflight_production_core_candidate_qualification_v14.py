"""Read-only V14 pre-consumption gate; never creates an attempt or runs a candidate."""
...
import audit_production_core_successor_attempt_authority_v14 as audit_authority
...
COMMIT = "0533f2ada34265d411f100beb441941e210d477a"
TREE = "13ac9ce7ce62d91063b12896ef8b7a21510b703b"
MIN_FREE_BYTES = 68_719_476_736
AUTHORITY = ROOT / "docs/artifacts/production-core-v14-successor-attempt-authority"
PUBLIC_FILES = (
...
def native_empty_output(path: Path, recovery_root: Path, private_root: Path, terminal_root: Path) -> dict[str, object]:
    ...
    if (resolved.is_relative_to(recovery_root.resolve(strict=True))
            ...
            or resolved.is_relative_to(terminal_root.resolve(strict=True))
...
def preflight(recovery_root: Path, private_root: Path, backup_root: Path, terminal_root: Path, unicode_root: Path, output: Path) -> dict[str, object]:
    from audit_production_core_v14_launcher_boundary import audit as audit_launcher_boundary

    launcher_boundary = audit_launcher_boundary()
    published_source_closure()
    output_state = native_empty_output(output, recovery_root, private_root, terminal_root)
    authority = audit_authority.audit(recovery_root, private_root, backup_root, terminal_root)
    validate_matrix()
    ...
    core: dict[str, object] = {
        "schema": "pastila-production-core-v14-preconsumption-preflight",
        "schema_version": 1,
        "result": "PASS",
...
def main() -> int:
    ...
    print(json.dumps(preflight(args.recovery_root, args.private_root, args.backup_root, args.terminal_root, args.unicode_root, args.output), sort_keys=True))
    return 0
