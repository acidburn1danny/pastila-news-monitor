"""Verify all Pilot 04 custodial proofs and atomically materialize ingestion."""
...
FREEZE_COMMIT = "a46ad48f3c0edb404b5466f0e77226e7a61d38ea"
PRE_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot04-preingestion-v1.json"
PACKET_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot04-signing-packet-v1.json"
...
PRIOR_LEDGER_PATH = "docs/artifacts/humor-mechanics-batch2-development-pilot03-ingestion-v1/access-ledger-segment.json"
SOURCE = ROOT / "owner-source-pilot04-v1.txt"
DECLARATION = ROOT / "owner-declaration-pilot04-v1.json"
DESTINATION = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot04-ingestion-v1"
...
PACKET_ID = "1c08073128a610fe3e21b30e75d74aabf0aa183d8baacc598ca4df3ccb1f2aa5"
HANDOFF = Path.home() / "Pastila-Owner-Handoff" / "Batch2-Development-Pilot04-v1"
...
def verify_signature(public_pem: str, algorithm: str, message: bytes, signature: bytes) -> bool:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot04-verify-") as temp:
        folder = Path(temp)
...
def spki_fingerprint(public_pem: str) -> str:
    with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot04-spki-") as temp:
...
def verify(responses: Path) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    if packet["packet_identity"] != seal("B2_DEVELOPMENT_PILOT04_SIGNING_PACKET_V1", packet["packet_core"]):
    ...
    paths = sorted(responses.glob("*.pilot04-response.json"))
    ...
    for path in paths:
        ...
        if seal("B2_PILOT04_SIGNING_CHALLENGE_V1", core) != identity:
        ...
        if not (
            ...
            and challenge["domain"] == "PASTILA_BATCH2_DEVELOPMENT_PILOT04_PREINGESTION_V1"
...
def materialize(pre: dict[str, Any], packet: dict[str, Any], verified: list[dict[str, Any]], prior_ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    source, declaration = SOURCE.read_bytes(), DECLARATION.read_bytes()
    response_set_identity = seal("B2_DEVELOPMENT_PILOT04_VERIFIED_RESPONSE_SET_V1", verified)
    previous = prior_ledger["final_ledger_head"]
    entries = []
    for sequence, operation in enumerate(packet["packet_core"]["operations"], start=36):
        ...
        entry = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                 ...
                 "event": "DEVELOPMENT_PILOT04_ATOMIC_INGESTION_OPERATION_ADMITTED",
    ...
    completion = {"schema_name": "batch2-custodial-access-ledger-entry-v1", "schema_version": "1.0.0",
                  "entry_sequence": 42, "previous_entry_hash": previous,
                  "event": "DEVELOPMENT_PILOT04_ATOMIC_INGESTION_COMPLETED",
    ...
    ledger = {"schema_name": "batch2-development-pilot04-access-ledger-segment-v1", "schema_version": "1.0.0",
              "previous_ledger_head": prior_ledger["final_ledger_head"], "entries": entries,
              "final_ledger_head": completion["entry_hash"]}
    ledger["ledger_segment_identity"] = seal("B2_DEVELOPMENT_PILOT04_ACCESS_LEDGER_SEGMENT_V1", ledger)
    ...
    archive_receipt = {"schema_name": "batch2-development-pilot04-immutable-archive-receipt-v1", "schema_version": "1.0.0",
                       "immutable_archive_commitment": pre["immutable_archive_commitment"],
    ...
    archive_receipt["archive_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT04_ARCHIVE_RECEIPT_V1", archive_receipt)
    evidence = {"schema_name": "batch2-development-pilot04-custodial-verification-v1", "schema_version": "1.0.0",
    ...
    evidence["verification_identity"] = seal("B2_DEVELOPMENT_PILOT04_CUSTODIAL_VERIFICATION_V1", evidence)
    receipt = {"schema_name": "batch2-development-pilot04-ingestion-receipt-v1", "schema_version": "1.0.0",
    ...
    receipt["ingestion_receipt_identity"] = seal("B2_DEVELOPMENT_PILOT04_INGESTION_RECEIPT_V1", receipt)
    temporary = Path(tempfile.mkdtemp(prefix=".pilot04-ingestion-", dir=DESTINATION.parent))
    try:
