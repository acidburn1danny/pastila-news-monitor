...
LAUNCHER_SHA="$(sha256sum "$(realpath -e -- "$0")" | cut -d' ' -f1)"; RUNNER_SHA="$(sha256sum "$RUNNER" | cut -d' ' -f1)"
...
env -i ADAPTER_SHA256="$9" CUBLAS_WORKSPACE_CONFIG=:4096:8 CUDA_VISIBLE_DEVICES=0 DEVELOPMENT_SHA256="${10}" HF_HUB_OFFLINE=1 LAUNCHER_SHA256="${12}" MODEL_SHA256="$8" PATH=/usr/bin:/bin PYTHONHASHSEED=0 RUNNER_SHA256="${13}" TOKENIZERS_PARALLELISM=false TRANSFORMERS_OFFLINE=1 TRITON_CACHE_DIR=/tmp/triton-cache TRITON_LIBCUDA_PATH=/usr/lib/wsl/lib /usr/sbin/chroot "$2" /opt/production-core-runtime/bin/python -I /tmp/input/authority/runner.py /tmp/input/model /tmp/input/adapter /tmp/input/authority/probes.jsonl /tmp/output "${11}"
' bash "$ROOTFS_TAR" "$ROOTFS" "$MODEL" "$ADAPTER" "$PROBES" "$OUTPUT" "$RUNNER" "$MODEL_SHA" "$ADAPTER_SHA" "$DEVELOPMENT_SHA" "$LABEL" "$LAUNCHER_SHA" "$RUNNER_SHA"
