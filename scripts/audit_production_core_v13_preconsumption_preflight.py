"""Fresh executable audit of both V13 pre-consumption gates, with zero writes."""
...
import audit_production_core_v13_launcher_boundary as signed_boundary
import preflight_production_core_candidate_qualification_v13 as gate
...
def audit(recovery_root: Path, private_root: Path, backup_root: Path, unicode_root: Path, output: Path) -> dict[str, str]:
    boundary = signed_boundary.audit()
    outer = gate.preflight(recovery_root, private_root, backup_root, unicode_root, output)
    command = [
        sys.executable, str(ROOT / "scripts/execute_production_core_candidate_qualification_v13.py"),
        "--preflight-only",
    ...
    if (receipt["execution_authority_identity"] != outer["authority_identity"]
        ...
        raise ValueError("V13 executable preflight or output state mismatch")
    stale = dict(receipt)
    ...
    return {
        ...
        "v13_authority_identity": outer["authority_identity"],
...
def main() -> int:
    ...
    print(json.dumps(audit(args.recovery_root, args.private_root, args.backup_root, args.unicode_root, args.output), sort_keys=True))
    return 0
