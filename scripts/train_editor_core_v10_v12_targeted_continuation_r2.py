"""Dedicated offline worker for the 72-row Editor Core targeted R2 round."""
...
EXPECTED_STEPS = 9
CHECKPOINT_STEPS = (9,)
EXPECTED_CONFIG_IDENTITY = "eb71032c79f9ba6769ac00915bd4316fdfa865d06d248bbfccfcfd94bcc7e1d6"
...
def validate_config(config: dict[str, object], corpus_sha256: str, row_count: int) -> dict[str, object]:
    ...
    random.Random(271828).shuffle(order)
...
def run_training(model_path: Path, parent_path: Path, corpus_path: Path, config_path: Path, output: Path) -> dict[str, object]:
    ...
    receipt = {"schema": "pastila-editor-core-targeted-r2-training-receipt", "schema_version": 1, **authority, "optimizer": "PAGED_ADAMW_8BIT_NEW_STATE", "optimizer_steps": steps, "checkpoint_identities": checkpoints, "final_loss": format(losses[-1], ".17g"), "network_activity": False, "holdout_used": False, "r4_used": False, "adjudication_performed": False}
...
def fixture_smoke(config: dict[str, object]) -> dict[str, object]:
    ...
    core = {"schema": "pastila-editor-core-targeted-r2-worker-fixture-smoke", "schema_version": 1, "status": "PASS_FIXTURE_ONLY", **plan, "model_loaded": False, "optimizer_created": False, "optimizer_steps_executed": 0, "training_performed": False}
