"""Fixture-only smoke for the R5 targeted training worker and bound route."""
...
FIXTURE = ROOT / "tests/fixtures/editor_core_targeted_training_route_r5/training-config.json"


def run() -> dict[str, object]:
    spec = importlib.util.spec_from_file_location("targeted_r5_worker", WORKER)
    ...
    required = ["unshare --kill-child=KILL --mount --net --pid --ipc --uts --fork", "TRAINING_EXECUTION_AUTHORIZED=1", "mount -o remount,bind,ro", "EXPECTED_SOURCE_COMMIT=e6ce011f8ba0b4c4d746b7dc0ee330845534d9e7", "EXPECTED_SOURCE_TREE=92412fae59efb64dd798ead55b79fb008c27ed30", 'GIT=(git -c "safe.directory=$REPOSITORY" -C "$REPOSITORY")', '"${GIT[@]}" merge-base --is-ancestor', "EXPECTED_ZERO_STEP=", "EXPECTED_WORKER=", "EXPECTED_CORPUS=", "--execute-authorized"]
