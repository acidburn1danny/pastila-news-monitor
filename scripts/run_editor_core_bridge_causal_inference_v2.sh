...
AUTHORITY="$REPOSITORY/docs/artifacts/editor-core-editorial-bridge-causal-inference-authority-v2.json"
VERIFIER="$REPOSITORY/scripts/verify_editor_core_bridge_causal_inference_authority_v2.py"; ROUTE="$REPOSITORY/scripts/run_editor_core_bridge_causal_inference_v2.sh"
...
setsid unshare --kill-child=KILL --mount --net --pid --ipc --uts --fork bash -s -- "$ROOTFS_TAR" "$ROOTFS" "$MODEL" "$ADAPTER" "$REQUESTS" "$OUTPUT" "$WORKER" "$CANDIDATE" "$SNAPSHOT" "$MODEL_SHA" "$ADAPTER_SHA" "$REQUESTS_SHA" "$WORKER_SHA" <<'CHILD' &
set -euo pipefail
...
for pair in "$5:requests.jsonl" "$7:worker.py"; do src="${pair%%:*}"; name="${pair#*:}"; touch "$2/tmp/input/authority/$name"; mount --bind "$src" "$2/tmp/input/authority/$name"; mount -o remount,bind,ro "$2/tmp/input/authority/$name"; done
