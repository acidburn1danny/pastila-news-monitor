"""Owner-operated signer for the frozen Pilot 04 unsigned packet.
...
PACKET = ROOT / "docs/artifacts/humor-mechanics-batch2-development-pilot04-signing-packet-v1.json"
...
PREINGESTION_ID = "f75c53645bb321ee821cbae4d65de6ed9701c4b61c14e67a403a4200387e1d22"
PRIOR_LEDGER_HEAD = "81e65fd0e0a703fe388a0391fad9eb94331c734a8e37ca98f1dd07c0faf38b56"
...
def verify_packet(packet: dict[str, Any]) -> None:
    if packet["packet_identity"] != PACKET_ID or seal("B2_DEVELOPMENT_PILOT04_SIGNING_PACKET_V1", packet["packet_core"]) != PACKET_ID:
        raise SystemExit("Pilot 04 packet identity mismatch")
    if packet["packet_core"]["preingestion_identity"] != PREINGESTION_ID or packet["packet_core"]["prior_ledger_head"] != PRIOR_LEDGER_HEAD:
        raise SystemExit("Pilot 04 lineage mismatch")
    if packet["status"] != "UNSIGNED" or packet["signatures_present"] != 0 or len(packet["signature_requests"]) != 8:
        raise SystemExit("Pilot 04 packet is not the frozen unsigned 8-request packet")
    nonces: set[str] = set()
    for item in packet["signature_requests"]:
        challenge = item["challenge"]
        core = {key: value for key, value in challenge.items() if key != "challenge_identity"}
        if challenge["challenge_identity"] != seal("B2_PILOT04_SIGNING_CHALLENGE_V1", core):
...
def sign_all(packet: dict[str, Any], key_dir: Path, response_dir: Path) -> None:
    ...
    for item in packet["signature_requests"]:
        ...
        output = response_dir / f"{item['operation_ordinal']:02d}-{stem}.pilot04-response.json"
        if output.exists():
        ...
        with tempfile.TemporaryDirectory(prefix="pastila-b2-pilot04-") as temporary:
        ...
        response = {
            "schema_name": "batch2-development-pilot04-custodial-signature-response-v1", "schema_version": "1.0.0",
