"""Fail-closed closure and contamination audit for targeted R4."""
...
def audit() -> dict:
    ...
    if tuple(map(len, (new, replay, training, holdout, keys))) != (12, 36, 48, 12, 12) or training != new + replay:
    ...
    if Counter(row["diagnostic_pattern"] for row in new) != Counter({"ATTRIBUTION_DENIAL_FINALITY": 8, "PRELIMINARY_CONTEST_PENDING": 4}):
    ...
    return {"verdict": "PASS", "blockers": 0, "manifest_identity": manifest["manifest_identity"], "training_config_identity": config["training_config_identity"], "new_rows": 12, "replay_rows": 36, "holdout_rows": 12, "replay_provenance": replay_provenance, "reference_rows_checked": len(references), "exact_contamination_matches": len(exact), "maximum_external_jaccard_5gram": maximum, "maximum_train_holdout_jaccard_5gram": internal, "frozen_holdout_training_use": False, "frozen_holdout_targets_reused": False, "rejected_r3_adapter_training_use": False, "r4_evidence_used": False, "token_audit_identity": token["audit_identity"], "maximum_training_sequence_tokens": token["maximum_training_sequence_tokens"], "training_performed": False}
