"""Tokenizer-only EOS and ceiling audit for the targeted R2 pack."""
...
def audit(model: Path, artifacts: Path) -> dict[str, object]:
    result = module().audit(model, artifacts)
    result["schema"] = "pastila-editor-core-targeted-r2-token-audit"
    core = {key: value for key, value in result.items() if key != "audit_identity"}
    result["audit_identity"] = module().sha(module().canonical(core))
    return result
