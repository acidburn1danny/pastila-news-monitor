...
[[ ${EDITOR_CORE_R5_TRAINING_OWNER_AUTHORIZED:-0} == 1 ]] || exit 3
...
ZERO_STEP=/mnt/c/pf9/scripts/launch_editor_core_v10_v12_targeted_r5_zero_step.py
REPOSITORY=/mnt/c/pf9
EXPECTED_SOURCE_COMMIT=e6ce011f8ba0b4c4d746b7dc0ee330845534d9e7
EXPECTED_SOURCE_TREE=92412fae59efb64dd798ead55b79fb008c27ed30
...
EXPECTED_CONFIG=50b3e03b7b10dabcaf46a75b8beb21ef7871cce746d831aa832f1785e72adcd4
EXPECTED_HELPER=7026fe0ea99d54d0fc6caefd5ac4b6c1119e488b1cdc96336ce14387b7ffb433
EXPECTED_ZERO_STEP=7866b0fee1418bb15ef337926e9ae8d8a8937388e11bb063567985bd5e238628
EXPECTED_WORKER=39f117b06daf837d7acc587b1169bfb6f388bfb7a05487a985578e3404b616f5
...
"${GIT[@]}" merge-base --is-ancestor "$EXPECTED_SOURCE_COMMIT" origin/successor/core-v2-v12-runner-binding-remediation || exit 4
...
python3 -B "$ZERO_STEP" --model "$MODEL" --parent-checkpoint "$PARENT_CHECKPOINT" --rootfs "$ROOTFS_TAR" --driver-snapshot "$SNAPSHOT" --output "$OUTPUT" >/dev/null
WORK="$(mktemp -d /tmp/editor-targeted-r5-train.XXXXXXXX)"; ROOTFS="$WORK/rootfs"; mkdir "$ROOTFS"; child=""
