...
def audit(recovery: Path, private: Path, backup: Path, v13_terminal: Path,
    ...
    if (authority.get("schema") != "pastila-production-core-v15-execution-bound-preflight-authority"
    ...
    if (raw != json.dumps(authority, ensure_ascii=False, sort_keys=True, indent=2).encode() + b"\n"
            or files["builder-source.py"].read_bytes() != (issuer.ROOT / "scripts/materialize_production_core_v15_execution_bound_preflight.py").read_bytes()
