"""Read-only closure and comparison audit for R5 holdout outputs."""
...
SEMANTIC_MARKERS = {
    "ATTRIBUTION_DENIAL_FINALITY": (
        ("suspect", "suspiciun", "acuza", "acuzat", "acusat"), ("ar fi", "posibil"),
        ("respinge", "neaga", "contesta"),
        ("nu a confirmat", "neconfirmat", "fara confirmare"),
        ("continua", "in curs", "neincheiat", "deschis"),
    ),
    "PRELIMINARY_CONTEST_PENDING": (
        ("sugereaza", "posibil", "ar fi"),
        ("contesta", "contestare", "respinge", "obiecteaza"),
        ("preliminar", "provizoriu"),
        ("neincheiat", "nu s-a incheiat", "continua", "in curs"),
    ),
}
...
def audit(output: Path, answer_key: Path, candidate: str, adapter_sha256: str, requests_sha256: str, runner_sha256: str) -> dict[str, object]:
    ...
    core = {
        "schema": "pastila-editor-core-targeted-r5-holdout-audit", "schema_version": 1,
...
def compare(left: Path, right: Path, answer_key: Path) -> dict[str, object]:
    ...
    core = {
        "schema": "pastila-editor-core-targeted-r5-holdout-comparison", "schema_version": 1,
