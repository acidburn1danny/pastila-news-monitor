...
PUBLIC_PEM_SHA256 = "29616718e9d17a3c88f630af52fee0ef7a9dc7adc519412c4870f06b63ca1cca"
SOURCES = (
...
def source_closure() -> dict[str, str]:
    if git("rev-parse", f"{COMMIT}^{{tree}}").decode() != TREE:
        raise ValueError("published source commit or tree mismatch")
    head = git("rev-parse", "HEAD").decode()
    published = git("rev-parse", "refs/remotes/origin/successor/core-v2-v12-runner-binding-remediation").decode()
    if not (ancestor(COMMIT, head) and ancestor(COMMIT, published)):
        raise ValueError("published recovery branch mismatch")
    if not (ancestor(head, published) or ancestor(published, head)):
        raise ValueError("local and published histories diverge")
    observed = {}
    for name in SOURCES:
    ...
    committed_public = subprocess.check_output(
        ["git", "show", f"{COMMIT}:docs/artifacts/production-core-v8-1-signing-public.pem"], cwd=ROOT,
    )
    if (PUBLIC_KEY.is_symlink() or digest(committed_public) != PUBLIC_PEM_SHA256
            or PUBLIC_KEY.read_bytes().replace(b"\r\n", b"\n") != committed_public):
